...
export function useEditorShell({ onSave }) {
  ...
  useEffect(() => {
    const onKeyDown = (e) => {
      ...
      else if (mod && e.key === 'b') { e.preventDefault(); const hasMod=st.modified||st.timelineModified.weather||st.timelineModified.wind||st.timelineModified.runway; if(hasMod){showModal(t('modal_unsaved_title'),<p>{t('modal_unsaved_body')}</p>,<div className="modal-actions-row"><button className="btn-cancel" onClick={hideModal}>{t('modal_btn_cancel')}</button><button className="btn-confirm" onClick={()=>{hideModal();useAppStore.setState({modified:false,timelineModified:{weather:false,wind:false,runway:false},selectedIndices:new Set()});st.setScreen('browser');}}>{t('modal_btn_discard')}</button></div>);}else{st.setScreen('browser');} }
