...
// ─── Polygon simplicity (Triangulator guard) ─────────────────────
// The game triangulates every area polygon with a constrained Delaunay
// triangulator and hard-fails the level load when the outline crosses itself
// (Unity: "ConstraintEdges[i] ... and ConstraintEdges[j] ... intersect!" →
// NullReferenceException). Every edit that reshapes an area outline must go
// through this guard so a bowtie can never be committed or saved.

function _segProperCross(p1, p2, p3, p4, eps) {
  const d = (p2.x - p1.x) * (p4.z - p3.z) - (p2.z - p1.z) * (p4.x - p3.x);
  if (Math.abs(d) < eps) return false; // parallel/collinear never "properly" crosses
...
/**
 * True when the closed polygon `points` has no self-crossings (a "simple"
 * polygon). Edges that merely share an endpoint are legal connections;
 * only interior×interior crossings disqualify. Degenerate (zero-length)
 * edges are ignored. A bbox pre-filter keeps large OSM outlines cheap
 * enough to call per pointermove.
 *
 * @param {Array<{x:number,z:number}>} points - open or closed ring (a
 *   trailing duplicate of the first point is tolerated)
 * @param {number} [epsilon=1e-9]
 * @returns {boolean}
 */
function polygonIsSimple(points, epsilon) {
  if (!Array.isArray(points) || points.length < 3) return true;
  const eps = epsilon != null ? epsilon : 1e-9;
  // Drop a duplicated closing point so the ring is vertex-only.
  const ring = points.slice();
  while (ring.length > 1 &&
         Math.abs(ring[ring.length - 1].x - ring[0].x) < eps &&
         Math.abs(ring[ring.length - 1].z - ring[0].z) < eps) ring.pop();
  const n = ring.length;
  if (n < 3) return true;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (Math.abs(a.x - b.x) < eps && Math.abs(a.z - b.z) < eps) continue; // degenerate
    edges.push({ a, b, minX: Math.min(a.x, b.x), maxX: Math.max(a.x, b.x), minZ: Math.min(a.z, b.z), maxZ: Math.max(a.z, b.z) });
  }
  for (let i = 0; i < edges.length; i++) {
    for (let j = i + 1; j < edges.length; j++) {
      // Adjacent through the ring (and first↔last through closure) share a vertex.
      if (j === i + 1) continue;
