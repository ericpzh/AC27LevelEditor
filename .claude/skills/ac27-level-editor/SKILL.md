...
- **Stale $fstrref cleanup on save:** After rebuilding StaticData.$blobdoc.StaticItems, `_rebuildStaticDataSections` scans the ACL text for `$fstrref:"flight-plan:REG"` references pointing to deleted registrations and replaces them with `null`. Also resets jetway docking state (`Status→0`, `Progress→0`, `DockingAircraft→null`, `DockingDoorIndex→-1`) in checkpoint frame RuntimeEntities for orphaned docking entries.
