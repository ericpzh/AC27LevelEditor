...
| **ACL Backend** | `src/acl/parser.js` | **Facade** — main.js imports ALL backend modules through it only (17 modules, CommonJS + some ESM). Exports `detectSchemaVersion()` for v2/v3 vs v4 routing |
| | `src/acl/config.js` | Centralized time resolution: `resolveConfigTime(rawText)` extracts Config block and overrides startTime with GameTime.CurrentDateTime. `resolveDisplayTimes()` for UI. Lazy-requires to avoid circular deps |
...
- **v4 designator mapping:** `buildDesignatorMapping` now cross-references `StaticItems` (flight-plan → Registration, AircraftType) with `RuntimeEntities` (aircraft → Specification → Designator) to build the `AircraftType → Designator` map for v4 files. Previously returned empty for v4 — now produces a complete map for spec lookup during save.
- **Specification extraction for v4:** `extractSpecificationDB` now scans the entire v4 decoded text for `Specification` objects inside jetway `DockingAircraft` entries and `RuntimeData` aircraft entries (not the traditional `WorldState.Aircrafts` section). A fallback `_extractFallbackSpec(text)` extracts a single spec from arbitrary text when the approachCache specDB/designatorMap are unavailable.
...
- **Stand heading from TailPosition→NosePosition:** v4 stands link to taxiway nodes via `$iref` references (`TailPosition`/`NosePosition`). The `_parseStandPositions` v4 path resolves these `$iref` values to coordinates and now computes the heading from the tail→nose vector (`Math.atan2(-dz, dx)`), matching the v2/v3 path output (`{ x, y, heading, tailX, tailZ, noseX, noseZ }`).
- **Save pipeline cleanup (v1.2.4+):** After rebuilding StaticData (step 7), `_rebuildStaticDataSections(aclPath, flights, undefined, approachCache, aclcfgStartTime, _saveSec)` runs a multi-stage cleanup across ALL segments (header blobdoc + checkpoint frames). Before modifications begin, `_validateStandConflicts()` checks all flights for stand occupancy conflicts (consecutive ARR→DEP pairs must have matching registrations — different aircraft cannot occupy the same stand simultaneously). Then each GATCARC4 segment is an independent Odin binary document with its own type numbering and `$id` namespace, so the pipeline creates **per-segment type maps** and **per-segment `_IdMapper` instances** for centralized `$iref` remapping. `aclcfgStartTime` and `_saveSec` are threaded through to `_rebuildFlightRuntimeEntities` for aircraft position computation. Execution order (critical — 7b MUST run before 7a so jetway detection sees intact `$fstrref` values):
  1. **7a-2. Pre-expand short-form $type refs:** Expands bare `$type: N` references to fully-qualified strings **before** any cleanup steps that might remove type declarations. Uses per-segment type maps so each segment's own declarations are used. Surviving bare refs in rebuilt jetway entries are re-expanded in 7b-2.
  2. **7b-1. Rebuild jetway entries constructively (`_rebuildJetwayEntries`):** Replaces the modification-based `_resetFrameJetwayDockingState` with a **constructive** two-pass approach. Pass 1 parses all RuntimeEntities entries structurally (tokenizer), building a stand→flight lookup from DEP flights (when multiple DEPs share a stand, keeps the earliest OffBlockTime). Also builds a stand→arrivals lookup for **turnaround detection**: when a stand hosts both an ARR and DEP for the same aircraft (matching registration) and the arrival lands before the departure off-block, the jetway is left empty (aircraft hasn't arrived at the gate yet). For DEP flights on matching stands, `_buildActiveJetwayEntry()` generates a complete ~35-field Aircraft structure inside DockingAircraft from a hardcoded template. For stale/empty jetways, a clean cleared-state template is used. Collects shared `AircraftEvent[]` and `ECommand[]` canonical `$id` values for shared `$iref` dedup across aircraft entries.
  3. **7b-2. Re-expand bare $type refs:** Re-runs `_expandShortFormTypes` on segments modified by 7b-1, since the new injected entries use bare `$type: N` integers (3-6) whose full-form declarations were in the original entries that were replaced.
  4. **7b-3. Rebuild flight-plan, aircraft, and animator RuntimeEntities (`_rebuildFlightRuntimeEntities`):** Deletes ALL existing `flight-plan:REG`, `aircraft:REG`, and `aircraft-animator:aircraft:REG` entries and rebuilds them from editor state in 4 passes. Pass 1: remove old entries, collect old `$id` for iref remapping. Pass 2: rebuild full inline `flight-plan:REG` entries with time fields from `_computeTakeoffTicks`/`_computeArrivalInBlockTicks`. Turnaround detection avoids duplicate aircraft entries when the same REG is used for both ARR and DEP (winner determined by earlier event time). Pass 3: build `aircraft:REG` entries for ARR flights via `_buildStandaloneAircraftEntry` — computes position/direction from approach path using `computePosition`/`computeDirection` (same State=30/State=5 algorithm as v2/v3). DEP flights at jetway stands get `$iref`-style entries pointing to the embedded `_flightPlan.$id`. Pass 4: build `aircraft-animator:aircraft:REG` entries. Ordering: flight-plans FIRST, then aircraft/animator, then kept entries (jetway, radio-channel, singleton) — so `$id` definitions precede `$iref` references.
  ...
  - **New constants:** `DEPARTURE_TAXI_SECONDS` and `ARRIVAL_TAXI_SECONDS` in `src/utils/constants/timing.js` provide per-airport overrides for estimated OffBlockTime→Takeoff and LandingTime→InBlockTime durations (defaults: 300s departure, 400s arrival). Used by `_computeTakeoffTicks` / `_computeArrivalInBlockTicks`. `TAXI_SPEED`, `POSITIVE_TAXI_ACCEL`, `NEGATIVE_TAXI_ACCEL` (also from `timing.js`, re-exported via `constants.js`) supply aircraft dynamics values for `_buildActiveJetwayEntry` and `_buildStandaloneAircraftEntry`.
...
- **GameTime.CurrentDateTime parsing:** v4 `extractCurrentDateTime` tries `GameTime.CurrentDateTime` first (WorldState snapshot, parsed from nested ticks), falls back to `MetaData.BaseTime`. Used by `resolveConfigTime` to override config startTime with post-warmup time.
...
16. **Use `resolveConfigTime()` for config time display.** Every consumer of ACL config time (startTime/endTime) must use `resolveConfigTime(rawText)` from `src/acl/config.js` — not `_extractConfig()` directly. `resolveConfigTime` applies the GameTime.CurrentDateTime override (post-warmup time). This is the single source of truth. All IPC handlers that read config (`get-file-info`, `load-acl`, `save-acl`, `import-zip`, `restore-latest-backup`, `load-timelines`) already use it.
...
19. **Bump `CACHE_VERSION` when cache.json schema changes.** Any change to the structure of `approachData`, `saveTimeOffsets`, `fileTypeMaps`, `state5ParamsMap`, `taxiwayPaths`, `sidPaths`, `missedAppPaths`, or new top-level keys in cache.json MUST bump `CACHE_VERSION` in `src/utils/constants/timing.js` (re-exported via `src/utils/constants/index.js` and `src/acl/constants.js` for CJS backward compat). Stale caches silently corrupt saves.
20. **Version tags for releases.** Version tags use `v<MAJOR>.<MINOR>.<PATCH>` (e.g. `v1.2.0`). Keep these three in sync: (a) `.claude/skills/ac27-editor/SKILL.md` Project Identity line, (b) `package.json` `version` field, (c) the git tag. The user decides when to bump; you can re-tag the same version to include new changes.
