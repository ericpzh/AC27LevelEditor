...
- **Save pipeline cleanup (v1.2.4+):** After rebuilding StaticData (step 7), `_rebuildStaticDataSections` runs a multi-stage cleanup across all segments (header blobdoc + checkpoint frames):
  - **7a. $fstrref cleanup & remapping:** Scans for `$fstrref:"flight-plan:REG"` references — replaces with `null` for deleted registrations, remaps to the new registration for renamed ones (detected by matching old CallSign to new flight CallSign).
  - **7a-2. Pre-expand short-form $type refs:** Expands bare `$type: N` references to fully-qualified strings **before** cleanup steps that might remove type declarations. This prevents "unknown type id N" encoding errors when a segment's only `N|TypeName` declaration lives in an entry that gets removed, and uses per-segment type maps (each GATCARC4 segment has independent type numbering).
  - **7b. Reset jetway docking state:** Resets jetway entries (`Status→0`, `Progress→0`, `DockingAircraft→null`, `DockingDoorIndex→-1`) for orphaned docking entries with stale `$fstrref` refs. Now uses structural `createTokenizer` parsing instead of regex, applied to ALL segments (header + frames), not just checkpoint frames.
  - **7c. Remove orphaned RuntimeEntities:** `_removeOrphanedFlightEntities()` removes/renames entries whose `$k` key (`flight-plan:REG`, `aircraft:REG`, `aircraft-animator:aircraft:REG`) references a registration that no longer exists in the rebuilt StaticItems. On registration rename, updates the `$k` value to the new registration. Handles fallback via `StaticItem.$fstrref` for already-corrupted saves.
  - **7d. Cleanup EventLog LatestEvents:** `_cleanupEventLogLatestEvents()` removes stale `aircraft:REG` keys from the event-log entity's nested `LatestEvents` dictionary, and handles registration renames there too. Invisible to step 7c because it lives deeper inside `singleton:event-log.$v.LatestEvents.$rcontent`.
  - **Registration renames:** When a flight's CallSign stays the same but its Registration changed, the pipeline detects this by matching old entries' CallSign (extracted from `InitialArrival`/`InitialDeparture`) to new flights' CallSign, building a `renameMap` that feeds into steps 7a/7c/7d to avoid unnecessary data loss.
- **Blobdoc scope-aware $type expansion:** The `_expandShortFormTypes` function was refactored into `_expandWithBlobdocScopes` (recursive scope-aware walker) + `_replaceBareTypeRefs` (single-scope regex expander). Each `$blobdoc` section has independent type numbering, so bare `$type: N` inside a blobdoc must expand using that blobdoc's own type declarations.
