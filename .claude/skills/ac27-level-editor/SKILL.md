...
- **Save pipeline cleanup (v1.2.4+):** After rebuilding StaticData (step 7), `_rebuildStaticDataSections` runs a multi-stage cleanup across ALL segments (header blobdoc + checkpoint frames). Each GATCARC4 segment is an independent Odin binary document with its own type numbering and `$id` namespace, so the pipeline creates **per-segment type maps** and **per-segment `_IdMapper` instances** for centralized `$iref` remapping. Execution order (critical — 7b MUST run before 7a so jetway detection sees intact `$fstrref` values):
  ...
  4. **7b-3. Rebuild flight-plan RuntimeEntities (`_rebuildFlightPlanRuntimeEntities`):** Deletes ALL existing `flight-plan:REG` entries and rebuilds them from editor state. DEP flights at jetway stands get `$iref`-style entries pointing to the `_flightPlan.$id` from `_buildActiveJetwayEntry`. All other flights get full inline entries with time fields populated via `_computeTakeoffTicks`/`_computeArrivalInBlockTicks` (which fall back to OffBlockTime/LandingTime + per-airport taxi constants when TakeoffTime/InBlockTime are 0 — the v4 default). Entries are ordered with flight-plans FIRST (before aircraft/animator/jetway) so the game processes arrivals and departures in correct interleaved order.
  ...
  6. **7d. Cleanup EventLog LatestEvents (`_cleanupEventLogLatestEvents`):** Removes stale `aircraft:REG` keys from the event-log entity's nested `LatestEvents` dictionary, and handles registration renames there too. Invisible to step 7c because it lives deeper inside `singleton:event-log.$v.LatestEvents.$rcontent`.
  ...
  - **Registration renames:** When a flight's CallSign stays the same but its Registration changed, the pipeline detects this by matching old entries' CallSign (extracted from `InitialArrival`/`InitialDeparture`) to new flights' CallSign, building a `renameMap` that feeds into steps 7c/7d/7a to avoid unnecessary data loss.
  - **New constants:** `DEPARTURE_TAXI_SECONDS` and `ARRIVAL_TAXI_SECONDS` in `src/utils/constants/timing.js` provide per-airport overrides for estimated OffBlockTime→Takeoff and LandingTime→InBlockTime durations (defaults: 300s departure, 400s arrival). Used by `_computeTakeoffTicks` / `_computeArrivalInBlockTicks`.
