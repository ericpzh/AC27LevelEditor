...
- `electron/main.js` manages three `Map` instances:
  - `groundMapWindows` â€” keyed by airport ICAO, holds `BrowserWindow` for Surface Radar
  - `airMapWindows` â€” keyed by airport ICAO, holds `BrowserWindow` for Approach Radar
  - `flightStripsWindows` â€” keyed by airport ICAO, holds `BrowserWindow` for Flight Strips
- Each map window loads the same Vite SPA with query params:
  - `?window=groundMap&airport=XXXX&root=...` â†’ renders `<GroundMapWindow>`
  - `?window=airMap&airport=XXXX&root=...` â†’ renders `<AirMapWindow>`
  - `?window=flightStrips&airport=XXXX&root=...` â†’ renders `<FlightStripsWindow>`
...
| `get-flight-strip-data` | `(airportIcao, gameRoot)` | invoke | Scans ACL files for callsignâ†’registration/airport/squawk mappings |
| `radar-window-closed` | `{ icao, type }` | mainâ†’renderer | Notifies main window that user closed a map window (X button) |
...
| `aircraft-selected-in-map` | `{ icao, callSign }` | mainâ†’renderer (push) | Broadcasts selection change to ALL map windows (ground + air + strips) for the same airport |
...
| `udp-aircraft-state` | `state` | mainâ†’renderer (push) | Live aircraft state pushed every 200ms to all open map windows |
...
| `emergency-aircraft-changed` | `{ icao, callSign }` | mainâ†’renderer (push) | Broadcasts EM change to ALL map windows (ground + air + strips) |

## Preload API (`window.electronAPI` additions)

```js
// Map window launchers
openGroundMap(airportIcao, gameRoot)    // â†’ ipcRenderer.invoke('open-ground-map', ...)
openAirMap(airportIcao, gameRoot)       // â†’ ipcRenderer.invoke('open-air-map', ...)
closeGroundMap(airportIcao)             // â†’ ipcRenderer.invoke('close-ground-map', ...)
closeAirMap(airportIcao)                // â†’ ipcRenderer.invoke('close-air-map', ...)
openFlightStrips(airportIcao, gameRoot)  // â†’ ipcRenderer.invoke('open-flight-strips', ...)
closeFlightStrips(airportIcao)          // â†’ ipcRenderer.invoke('close-flight-strips', ...)
getFlightStripData(airportIcao, gameRoot) // â†’ ipcRenderer.invoke('get-flight-strip-data', ...)
onRadarWindowClosed(cb)                 // â†’ ipcRenderer.on('radar-window-closed', handler)

// Linked aircraft selection (synced across ground + air map for same airport)
selectAircraftInMap(airportIcao, callSign)  // â†’ ipcRenderer.invoke('select-aircraft-in-map', ...)
getSelectedAircraft(airportIcao)            // â†’ ipcRenderer.invoke('get-selected-aircraft', ...)
onAircraftSelectedInMap(cb)                 // â†’ ipcRenderer.on('aircraft-selected-in-map', handler)
offAircraftSelectedInMap(cb)                // â†’ ipcRenderer.removeListener(...)

// Emergency aircraft (EM label â†’ squawk 7700)
setEmergencyAircraft(airportIcao, callSign) // â†’ ipcRenderer.invoke('set-emergency-aircraft', ...)
getEmergencyAircraft(airportIcao)           // â†’ ipcRenderer.invoke('get-emergency-aircraft', ...)
onEmergencyAircraftChanged(cb)              // â†’ ipcRenderer.on('emergency-aircraft-changed', handler)
offEmergencyAircraftChanged(cb)             // â†’ ipcRenderer.removeListener(...)

// UDP telemetry
getUdpStatus()                          // â†’ { connected, lastPacketTime, currentAirport, simFlags, heartbeatSeq }
getUdpAircraftState()                   // â†’ { aircraft, currentAirport, recordCount, simTimeUnixMs, simFlags, timeScale }
resetUdpAircraft()                      // â†’ clears all aircraft state (map refresh button); also resets lastHasLevel
sendUdpCommand(commandId, callSign)     // â†’ base64-encodes 12B callSign, invokes 'send-udp-command'
debugLog(...args)                       // â†’ ipcRenderer.invoke('debug-log', args) â€” logs to main terminal
...
**Data sources:**
- `_taxiwayPaths` â€” taxiway centerline polylines from approach cache (via `electronAPI.collectValues()`)
- `_runwayData` â€” runway rectangles (threshold pairs + width) computed in `collect-values` IPC
- `_standPositions` â€” stand midpoints from approach cache (via `electronAPI.collectValues()`)
- `_areaData` â€” area polygons by AreaType (0=airport boundary, 1=stand/apron, 2=building) from approach cache
- `useUdpAircraftState()` â€” live aircraft positions + `simTimeUnixMs` from UDP telemetry
- `GROUND_MAP_CENTER_OFFSET` â€” per-airport viewBox center offset (game units)

**Rendering layers:**
1. Radar-blue background (`#0a1628`). Witch mode: `witch/groundradar.png` image stretched to viewBox, background color `#24150a`.
2. Taxiway centerlines â€” uniform grey (`#444`) polylines. Segments touching stand-position nodes are marked `isStandAccess: true` and rendered with square linecap + configurable width multiplier (`GROUND_MAP_STAND_ACCESS_WIDTH_MULT`). Stand-access segments are no longer excluded â€” they render alongside main taxiways for differentiated styling. **Runway-named taxiway segments** (name matches a runway in `runwayData`) are excluded from this layer â€” they render as runway-style polygons instead (see layer 4b).
3. Area polygons â€” semi-transparent fills by AreaType: blue boundary, grey apron, black buildings. Default stroke color `#444` (matches taxiways). Parsed from `SceneryData.Areas` via `_parseAreas()`.
4. Runway rectangles â€” black filled polygons from threshold endpoints + width
5. **Runway-named taxiway segments** â€” taxiway centerlines whose name matches a runway entry are rendered as black filled polygons (same style as runways), using `computeRunwayCorners()` with the matching runway's width. These represent runway surfaces stored as taxiway centerline segments.
6. Taxiway labels â€” name labels at path midpoints with proximity dedup (`GROUND_MAP_TAXIWAY_LABEL_SPACING`). Placed **above** runways in layer order. Only rendered for non-runway taxiway segments.
7. Live ground aircraft â€” filtered to `position.y <= 1.0` (ground-level, not airborne) with inactive aircraft hidden by default:
   - **Inactivity filter:** Aircraft are hidden when parked â€” determined by `controlSeat` (UDP v2 record field at offset 21). If `controlSeat` is None (0) or Unknown (255), the aircraft has no active controller â†’ parked/hidden. If `controlSeat` is 1-7 (Ramp/Ground/Tower/Departure/Approach/Delivery/Apron), the aircraft is under active control â†’ always shown, even when at a stand. As a fallback (pre-v2 UDP data without `controlSeat`), aircraft at a known stand within `GROUND_RADAR_STAND_PROXIMITY` (0.5 GU â‰ˆ 50m) are hidden.
   ...
   - **Witch mode (v1.1.5):** Double-click the help `?` button to toggle. Aircraft rendered as animated 2-frame sprites from 15 character sheets (`public/witch/*.png`, each a 1536Ã—768 sprite sheet with 18 cells in a 3-rowÃ—6-column grid of 256Ã—256 PNGs with transparent backgrounds). A nested `<svg>` with `clipPath` isolates the target cell, then an `<image>` loads the full sheet clipped to that cell. `feDropShadow` traces the character's alpha channel for a white silhouette glow â€” only on the **active** (click-selected) aircraft (`callSign === selectedCallSign`). **Sprite assignment is centralized in the main process:** `witchSpriteMap` (Map<callSign, spriteIndex>) assigns each callsign a stable 0â€“14 index round-robin. The `spriteIdx` is injected into each aircraft object during the 200ms UDP push (`electron/main.js`), guaranteeing all windows show the same character. `witchMode.js` accepts `spriteIdx` as a parameter to `getSpriteSheet()`; without it (standalone/testing), falls back to a deterministic djb2 hash of the callsign. Moving: walk sprites (direction-aware via `witchDirection()`); parked/stopped: stand sprites (`isParked()` uses `controlSeat` â€” None (0) or Unknown (255) = parked; any active seat (1-7) = not parked). Airport boundary (AreaType 0) is hidden. Any click exits witch mode. Labels and connector lines hidden. Background replaced with `witch/groundradar.png`, sidebar gets witch-themed UI (bar.png background, button.png/button_on.png toggles, knob.png spin knobs, help.png icon).
...
**Data sources:**
- `_starPaths` (STAR routes, Type=0) â€” rendered in grey; trimmed at APPR overlap points
- `_sidPaths` (SID departure routes, Type=2) â€” rendered in grey
- `_missedAppPaths` (Missed Approach routes, Type=3) â€” rendered in grey
- `_apprPaths` (RNAV approach routes, Type=1) â€” rendered in grey; points used to trim STAR display
- `_runwayThresholds` from approach cache â€” for threshold lines and runway extensions
- `useUdpAircraftState()` â€” live aircraft positions + `simTimeUnixMs`
- `AIR_MAP_BG_OFFSETS` from `src/utils/constants.js` â€” per-airport background image config
- `AIR_MAP_DEFAULT_ZOOM` from `src/utils/constants.js` â€” per-airport initial zoom scale
- `NM_TO_GU` from `src/utils/constants.js` â€” nautical mile to game-units conversion (18.52)
...
5. Runway extension lines (toggleable): 1â€“20 NM dashed white lines from each threshold with tick marks at 5/10/15/20 NM.
6. Runway thresholds â€” runway-width lines connecting threshold pairs.
7. Border overlay â€” independent SVG with white border rect and 10Â° tick marks with degree labels. Tick/label sizes scale inversely to container width via `ResizeObserver` (baseline 800px) so they stay fixed in pixels when the window resizes.
8. Live airborne aircraft â€” filtered to `position.y > 1.0`:
   ...
   - **Heading line:** For selected aircraft only, projects nose direction forward 12Ã— planeScale
   - **Label:** By default, Tower aircraft and selected aircraft show full label (callsign + altitude + speed/type); other aircraft show altitude only. The ARR/DEP toggles on the left RunwaySidebar override this â€” when active, all aircraft of that direction show the full label. Speed/type toggles every 5 seconds between airspeed/10 and aircraft type. Dynamically positioned via anti-overlap layout (4 candidate positions: right/top/left/bottom). Emergency aircraft show a red "EM" label â€” above the callsign in full-label mode, above the altitude in altitude-only mode (v1.1.6).
...
**Airspace knob:** `SpinKnob` passed via `airspaceKnob` prop to `ControlSidebar` â€” controls range ring density (0=10NM gap â€¦ 11=120NM gap, default 40NM). Double-click knob to reset to default.
...
**RunwaySidebar:** Vertical bar on left (60px black). **Top section:** ARR/DEP label toggle buttons (default off) â€” when active, all aircraft of that direction show full labels (callsign + altitude + type) instead of just altitude. **Bottom section:** one RWY-prefixed toggle per runway, stacked from bottom. Both sections reuse `.air-map-toggle` classes for witch mode sprites. Only runways with resolved path data appear. Each runway and the ARR/DEP buttons get dynamic entries in the help overlay.
...
**Layout:** Horizontal row of columns with a bottom bar: sim clock + game speed multiplier (Ã—1/Ã—2 from UDP `timeScale`), refresh (portal tooltip from map help i18n, gated by `MAP_TOOLTIPS_ENABLED`), help (no tooltip â€” doubles as witch-mode toggle). Runway separator bars have solid black (`#000`) background. i18n: strips use hardcoded English only (seat labels, headers, runway separators never translated); help overlay has full i18n.

**Data sources:**
- `useUdpAircraftState()` â€” live aircraft + `simTimeUnixMs` + `timeScale` + `udpAirportChanged` from UDP
- `electronAPI.getFlightStripData()` â€” registration/airport/airway/squawk from ACL files
- `electronAPI.onAircraftSelectedInMap()` â€” cross-window selection sync (broadcast now includes strips)

**Strip layout (5 sections):**
1. **Callsign column** â€” bordered callsign box + aircraft type + stand label
2. **Procedure column** â€” STAR/SID procedure + registration + destination/origin airport
3. **Squawk column** â€” 4-digit squawk code (deterministic hash of callsign, 2000â€“6000)
4. **Route column** â€” stacked route history (fills remaining width, flex-column)
5. **Runway column** â€” runway designator + seat channel box (e.g. "GND", "TWR")
...
- `telemetryStatus` from UDP v2 records (offset 23) drives CSS modifier classes:
  - `2` (ActionRequired) â†’ `strip-telemetry-action-required` â€” muted border via `color-mix(in srgb, var(--orange/blue) 50%, #000)`
  - `3` (HandoffPending) â†’ `strip-telemetry-handoff-pending` â€” channel box gets `var(--accent)` border + `var(--accent-dim)` background
  - `4` (PendingAtStand) â†’ `strip-telemetry-pending-stand` â€” same channel box highlight
...
- `routeHistory` state: `{ callsign: [{ text, struck }] }` â€” tracks taxiway/airway changes
...
**Selection sync:**
- Click toggles; broadcasts via `select-aircraft-in-map` â†’ `broadcastSelectedAircraft()` sends to ground + air + strips
- Selected strips scale up (1.20Ã—) with solid backdrop (`#2a1a05` arr / `#0a1a2a` dep)
- **Dynamic transform-origin (v1.1.6):** `useLayoutEffect` in `FlightStripContent` computes per-strip `transformOrigin` based on viewport edge detection, preventing the 1.20Ã— scaled strip from overflowing the window. Grows away from overflowing edges (e.g., if right edge overflows â†’ `originX = 'right'`).
- `selectedCallSignRef` keeps stable `handleDragEnd` in sync for correct toggle/deselect IPC

**Drag reorder (v1.1.6 â€” runway-group constrained):**
- Long-press (400ms) enters drag mode
- **Runway-group constraint:** Drag targets are validated against the source strip's runway group. `runwayRanges` (memoized per seat) maps each runway â†’ `{ start, end }` flat indices. A drop is only valid if `hoverIdx` falls within the source runway's range, at `end+1` (end of group), or at the very end when source is the last group.
- Invalid targets (cross-runway drops) snap back immediately (no animation, selection cleared)
- Valid drops trigger `isDropping` state â†’ drop animation plays â†’ selection cleared on animation end
- **Drop animation (v1.1.6):** Double-rAF waits for React re-render with new strip order, then animates ghost from mouse position to the strip's new DOM position. Ghost gets `.strip-dropping` class: `transition: top 0.22s, left 0.22s, transform 0.22s, opacity 0.18s` â€” scales to 1.0, fades to opacity 0. Falls back to 400ms timeout if `transitionend` doesn't fire. Cleanup: cancels rAF frames, removes class.
- Pixel-level ghost tracking via direct DOM (`ghostRef`) â€” no React re-render; only `hoverIdx` changes trigger `setDragState`. Drag metadata in `dragMetaRef` (now includes `srcRunway`).
...
- **Animation:** 2-frame sprite animation at 500ms per frame via `setInterval` (`witchFrame` toggles 0â†”1). Timer is cleaned up on unmount or when witch mode is disabled.
- **Sprite rendering:** Each strip and drag ghost renders an inline `<svg>` (48Ã—48) inside a `.strip-witch-sprite` container (flex, left-aligned, 30px left padding). Same `clipPath` + `<image>` pattern as ground/air maps â€” loads the assigned sprite sheet, clips to the correct cell, and applies `feDropShadow` glow on selected aircraft only.
- **RPG stats (v1.1.6):** In witch mode, each strip and drag ghost displays HP/MP/ATK/DEF stats computed by `computeWitchStats()` â€” HP from first 2 digits of callsign (if "00" â†’ 100), MP from last 2 digits (if "01" â†’ 1), ATK from `airSpeedKnot / 10`, DEF from `position.y / 0.3048` (altitude in feet). Stats render in a 2Ã—2 CSS Grid (`.strip-witch-stats`) with gold labels (`.witch-stat-label`) and white values (`.witch-stat`). Drag ghosts also show stats.
- **Action selection:** Airborne (`position.y > 1.0`) â†’ `'fly'` sprites; parked on ground (`isParked()` via `controlSeat`) â†’ `'stand'` sprites; otherwise â†’ `'walk'` sprites with direction from `witchDirection(noseDirection)`.
- **Strip theming:** `.flight-strips.witch-mode` class on root enables themed CSS:
  ...
  - All text columns hidden (`.strip-col-callsign`, `.strip-col-proc`, `.strip-col-squawk`, `.strip-col-route`, `.strip-col-runway` â†’ `display: none`)
  ...
  - Drag ghost: themed backgrounds, scale 1.08Ã—, fades to opacity 0 with scale 1.0 on drop
  ...
  - Refresh/help buttons show `witch/refresh.png` and `witch/help.png` images (22Ã—22)
...
- Unique across all callsigns (collision-free), range 2000â€“6000
...
**Help overlay:** `MapHelpOverlay type="strips" title="Map Help"` â€” 3 sections: Buttons (Refresh, Help), Display (seat columns, runway separators, arrival/departure colors), Interaction (click to select, deselect, drag reorder). Full i18n (zh + en) for overlay content; `title` prop forces English header.
...
### Strip Command Interface (v1.1.7 â€” planned, UI hidden)
...
Branch commands (marked with `â†’`) navigate to a sub-menu showing dynamic options (runway designators or taxiway names). Leaf commands send a UDP command to the game and dismiss the bar.

**Data files:** `src/components/MapWindows/commandTree.js` (command definitions), `FlightStripCommandBar.jsx` (UI component), `src/utils/constants.js` (`CMD_*` constants 22â€“47).

**Taxiway names** are fetched via `collectValues` IPC â†’ `_taxiwayPaths.paths[].name` on mount and passed to `setTaxiways()` so sub-menus can generate dynamic taxiway options.

#### Command Table

| Seat | Aircraft State | Commands |
|------|---------------|----------|
| **RAMP** (1) | Ground | PUSH BACK APPR, CHANGE RWY TO â†’, CONTACT GND |
| **GROUND** (2) | Ground | PUSH BACK APPR, CHANGE RWY TO â†’, TAXI VIA â†’, HOLD SHORT â†’, DISPATCH TOW VIA â†’, CONTACT TWR, STAND BY, HOLD POSITION |
| **TOWER** (3) | DEP â€” Ground | CLEAR FOR TKOF, LINE UP & WAIT, HOLD SHORT â†’, CHANGE RWY TO â†’, TAXI VIA â†’, HOLD SHORT â†’, DISPATCH TOW VIA â†’, STAND BY, CROSS RWY, CONTACT DEP, CONTACT GND |
| **TOWER** (3) | ARR â€” Airborne | CLEARED TO LAND, GO AROUND, CONTINUE APPR, SELECT EXIT AT â†’ |
| **TOWER** (3) | ARR â€” Ground | SELECT EXIT AT â†’, CHANGE RWY TO â†’, CROSS RWY, CONTACT DEP, CONTACT GND |

#### Sub-Menu Sources

| Sub-Menu | Options From | Used By |
|----------|-------------|---------|
| `â†’` (runway) | `ac.runway` split on `/` (e.g. "13L/31R" â†’ ["13L", "31R"]) | HOLD SHORT (TOWER DEP), CHANGE RWY TO (all seats) |
| `â†’` (taxiway) | `_taxiwayPaths` from airport scenery cache, unique sorted names (A, B, C, A1â€¦) | TAXI VIA, HOLD SHORT, DISPATCH TOW VIA, SELECT EXIT AT |
...
> **Note:** Command IDs 22â€“23 are confirmed correct (used in `src/acl/flight_plans.js:805`). IDs 24â€“47 are placeholders pending game protocol verification.

### Voice Command Input (v1.1.7 â€” planned, UI hidden)
...
**Flow:**
```
PTT pressed â†’ clear selection â†’ capture speech â†’ detectLanguage (EN/ZH)
  â†’ parseCallsign: airline name â†’ ICAO code (via AIRLINE_CODE_MAP) + spoken numbers â†’ digits
  â†’ match against live UDP aircraft list â†’ select aircraft
  â†’ findBestCommandMatch: fuzzy match remaining text against getCommandsForAircraft()
  â†’ execute command via handleCommandAction()
```
...
| File | Purpose |
|------|---------|
| `voiceNumberParser.js` | `parseEnglishFlightNumber()` / `parseChineseFlightNumber()` â€” spoken numbers â†’ digit candidates. Handles "eleven eleven"â†’1111, "triple one"â†’111, "å¹ºå¹ºå¹ºå¹º"â†’1111, "æ´žå››"â†’04. Cartesian product for ambiguous cases. |
| `voiceCallsignParser.js` | `detectLanguage()` â€” CJK character check. `parseCallsign()` â€” longest-match airline name prefix (via `getSpokenToCode()` built from `AIRLINE_CODE_MAP`), then number parsing, then aircraft lookup. Separate EN and ZH paths (ZH uses character-level matching since no spaces). |
| `voiceCommandMatcher.js` | `findBestCommandMatch()` â€” two-stage: (1) exact alias lookup against hand-curated `EN_ALIASES`/`ZH_ALIASES` maps, (2) token Jaccard similarity with partial word overlap for English, bigram Dice coefficient for Chinese. `MATCH_THRESHOLD = 0.55`. `buildSpeechGrammar()` â€” JSGF grammar for SpeechGrammarList. |
| `useVoiceCommands.js` | React hook â€” manages `SpeechRecognition` lifecycle, silence timeout (2s auto-stop), cooldown (500ms). Orchestrates: transcript â†’ language detection â†’ callsign parsing â†’ command matching. Returns `{ listening, transcript, matchedCallsign, matchedCommand, confidence, isSupported, error, startListening, stopListening }`. |
| `VoicePTTButton.jsx` | Hold-to-talk mic button with 4 visual states: idle (gray `IoMicOutline`), listening (red `IoMic` + pulse animation), matched (green flash 300ms), error (dimmed red). Supports `witchMode` prop â€” renders `witch/voice.png`. |
...
- `useEffect` on `voice.matchedCallsign` â†’ calls `setSelectedCallSign()` + `selectAircraftInMap()`
- `useEffect` on `voice.matchedCommand` â†’ calls `electronAPI.sendUdpCommand()` + clears selection
...
- Scroll-wheel zoom: cursor-centered, 1.12Ã— factor per tick, clamped between 2% and 100% of initial viewBox
...
- **Imperative zoom/pan API** (for sidebar spin knobs, uses `viewBoxRef` to avoid stale closures):
  - `zoomIn()` / `zoomOut()` â€” center-based, 1.12Ã— factor, clamped
  - `panLeft()` / `panRight()` / `panUp()` / `panDown()` â€” 5% step, clamped to initial bounds
...
- Each aircraft object includes `spriteIdx` (0â€“14) injected by the main process during the push interval â€” used by witch mode for cross-window consistent character assignment
...
- **Toggle handler:** Checks `openGroundRadarAirports` / `openAirRadarAirports` / `openFlightStripAirports` Sets â€” if ICAO present, calls `closeXxxMap` IPC; otherwise calls `openXxxMap` IPC. Updates zustand state on both paths.
...
isGroundRadarOpen(icao)         // â†’ openGroundRadarAirports.has(icao)
isAirRadarOpen(icao)            // â†’ openAirRadarAirports.has(icao)
setFlightStripOpen(icao, open)  // Add/remove from openFlightStripAirports Set
isFlightStripOpen(icao)         // â†’ openFlightStripAirports.has(icao)
...
| `flight_strips_loading` | åŠ è½½ä¸­â€¦ | Loadingâ€¦ |
| `flight_strips_waiting` | ç­‰å¾…æ•°æ®â€¦ | Waiting for dataâ€¦ |
| `flight_strips_empty` | æ— æ´»è·ƒé£žæœº | No active aircraft |
| `flight_strips_runway` | è·‘é“ | RUNWAY |
| `seat_1`â€“`seat_7` | RMP/GND/TWR/DEP/APPR/DEL/APN | RMP/GND/TWR/DEP/APPR/DEL/APN |
| `seat_1_full`â€“`seat_7_full` | RAMP/GROUND/TOWER/DEPARTURE/APPROACH/DELIVERY/APRON | RAMP/GROUND/TOWER/DEPARTURE/APPROACH/DELIVERY/APRON |
...
// Extract description from a help i18n string (strips {{btn:...}} â€” prefix)
const helpTip = useCallback(
  (key) => t(key).replace(/^\{\{btn:\w+\}\}\s*[â€”â€“\-ï¼š:]\s*/, ''), [t]);

// Gated bind wrapper â€” returns {} when disabled (zero overhead)
...
**SpinKnob** â€” accepts optional `tooltip` string prop. Uses `useTooltip` internally, spreads `bind(tooltip)` on root `.spin-knob` div. Render `{TooltipPortal}` after the label. All gated by `MAP_TOOLTIPS_ENABLED`.

**RunwaySidebar** â€” accepts optional `arrTooltip`, `depTooltip`, `getRunwayTooltip(rwy)` props. Uses `useTooltip` internally. Replaces old `title` attributes with portal tooltips. All gated by `MAP_TOOLTIPS_ENABLED`.

**ControlSidebar** â€” accepts optional `knobTooltips` prop: `{ zoom?, panH?, panV?, airspace? }`. Passes tooltip strings to each `SpinKnob`. For `airspaceKnob` (injected as a React element), uses `React.cloneElement` to add the `tooltip` prop when enabled.

### Exclusions

- The **help/witch-mode toggle button** (`map-help-btn` / help `strips-bar-btn`) never gets a tooltip â€” it already has native `title` attributes and its double-click behavior toggles witch mode.
...
- **`NM_TO_GU`** (`src/utils/constants.js`): Nautical mile to game-units conversion (18.52 = 1852m Ã· 100 m/unit). Used by AirMapWindow for runway extension lines, tick marks, and range rings.
...
- **`GROUND_RADAR_STAND_PROXIMITY`** (`src/utils/constants.js`): Max distance (0.5 GU â‰ˆ 50m) from aircraft position to its assigned stand midpoint to consider it "parked at stand." Used by GroundMapWindow to hide inactive aircraft.
