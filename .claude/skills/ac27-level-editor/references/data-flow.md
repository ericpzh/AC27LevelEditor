...
3. `save-acl` IPC → sorts flights → looks up approach cache for the airport → generates full ACL via `_rebuildWorldStateSections()`:
   ...
   - **Multi-stage cleanup (steps 7a-2 through 7a) across ALL segments (header + frames),** with per-segment type maps and `_IdMapper` instances for centralized `$iref` remapping. Execution order is critical — 7b runs before 7a so jetway detection sees intact `$fstrref` values:
     ...
     - **7b-1. Constructive jetway rebuild (`_rebuildJetwayEntries` + `_buildActiveJetwayEntry`):** Replaces regex-based `_resetFrameJetwayDockingState` with two-pass constructive template generation. DEP flights at matching stands get a ~35-field Aircraft structure inside DockingAircraft; stale entries get a clean cleared-state template. Shares canonical `AircraftEvent[]` / `ECommand[]` `$id` values across aircraft entries.
     - **7b-3. Rebuild flight-plan RuntimeEntities (`_rebuildFlightPlanRuntimeEntities`):** Deletes ALL existing `flight-plan:REG` entries and rebuilds from editor state. DEP flights at jetway stands get `$iref`-style entries; all others get full inline entries with time fields from `_computeTakeoffTicks`/`_computeArrivalInBlockTicks` (taxi-fallback-aware for v4's zero defaults). Flight-plans placed FIRST in `$rcontent` so the game processes arrivals/departures in correct order.
     ...
     - **7d. Cleanup EventLog LatestEvents (`_cleanupEventLogLatestEvents`):** Removes stale `aircraft:REG` keys from the nested `singleton:event-log.$v.LatestEvents.$rcontent` dictionary -- invisible to step 7c due to its nesting depth.
