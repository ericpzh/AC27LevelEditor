...
- Preload `sendPatchCommand(patch)` → `send-patch-command` IPC handler → builds the 0x00E7 frame from `{ type, callSign, dx?, dy?, rate?, kts?, appr? }` (`rate` = smooth-turn °/s of game time; on update_heading it's the 5th positional field, on clear_for_appr it's sent as the keyed field `rate=N` — the plugin's cfa parser treats any bare numeric as the kts approach speed) (types: `update_heading` | `update_position` legacy alias | `clear_for_appr`)
