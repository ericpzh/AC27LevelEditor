...
- **Supported commands:**
  - commandId=1 (`SelectAircraft`), 12-byte ASCII callSign payload (20B total datagram). A callsign starting with `!` is hijacked by the AC27Appoarch plugin (Mechanism A — see the `ac27-appoarch` skill): `!5:<callsign>` = clear_for_appr, no selection happens
  - commandId=0x00E7 (**extended frame**, consumed by the AC27Appoarch plugin only): pipe-delimited ASCII payload, NUL-padded to **exactly 64 bytes** (72 B total datagram) — the plugin reads the datagram back from the game's receive buffer post-tick, so the fixed-length NUL-padded payload is a hard contract. Commands: `update_heading|CS|dx|dy` (heading-only override, no speed), `clear_for_appr|CS[|kts][|appr][|native=0]`, `track|CS` (diagnostics toggle). Never route `!` frames through `select-aircraft-in-map` (selection side-effects)
...
- Preload `sendPatchCommand(patch)` → `send-patch-command` IPC handler → builds the 0x00E7 frame from `{ type, callSign, dx?, dy?, kts?, appr? }` (types: `update_heading` | `update_position` legacy alias | `clear_for_appr`)
