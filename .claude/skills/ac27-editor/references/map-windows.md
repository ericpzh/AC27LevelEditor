...
| `electron/pttShortcut.js` | Main-process global-hotkey manager (CJS, deps injected — no `require('electron')` — unit-tested in `tests/electron/ptt-shortcut.test.js`): `createPttShortcutManager({globalShortcut, loadConfig, saveConfig, getStripsWindows, onTrigger})` — register/unregister/dispose, `get`/`set` backing (`pttShortcut` in `ac27-config.json`), focused-first strips routing. Wired in `electron/main.js` (`get-ptt-shortcut`/`set-ptt-shortcut` IPC, registered at `whenReady`, `unregisterAll` at `will-quit`) + `electron/preload.js` (`getPttShortcut`/`setPttShortcut`/`onGlobalPttToggle`). |
