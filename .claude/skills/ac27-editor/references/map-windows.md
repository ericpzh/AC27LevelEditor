...
2. Taxiway centerlines — uniform grey (`#444`) polylines. Segments touching stand-position nodes are marked `isStandAccess: true` and rendered with square linecap + configurable width multiplier (`GROUND_MAP_STAND_ACCESS_WIDTH_MULT`). Stand-access segments render alongside main taxiways for differentiated styling. **Runway-named taxiway segments** (name matches a runway in `runwayData`) are excluded from this layer — they render as runway-style polygons instead (see layer 4b).
...
**Zoom/pan:** `useSvgZoom` hook, per-airport initial viewBox via `GROUND_MAP_DEFAULT_ZOOM` + `GROUND_MAP_CENTER_OFFSET`, pan clamped to initial bounds. **Auto-fit over fetched geometry:** `contentViewBox` is computed from the fetched taxiway/stand/runway/area geometry (all points across `taxiwayPaths`, `standPositions`, `runwayData` thresholds, `areaData` polygons, ±10% padding) and takes precedence over the fixed origin-centered box — airports authored far from the world origin (e.g. ZGSZ) would otherwise render blank. `useSvgZoom` only applies the initial viewBox once at mount, so a `snapDoneRef` + `resetZoom()` effect re-centers the radar on the airport the first time geometry arrives.
...
**Layout:** Flex row with a `ControlSidebar` on the right containing spin knobs (zoom, E-W pan, S-N pan, airspace with gauge indicators), push-button toggles (STAR, SID, APPR, Labels, ILS, Refresh), and a **help button** (`?` icon). The approach radar renders no airport background image (witch mode's `witch/{ICAO}.png` layer is unaffected). All controls (except the help/witch button) have on-hover portal tooltips sourced from the map help page i18n content, gated by `MAP_TOOLTIPS_ENABLED`. The left `RunwaySidebar` (Waypoints above ARR, DEP, per-runway toggles) also has tooltips. Sim-time clock in top-left corner.
...
1. Witch-mode background image only: `witch/{ICAO}.png` at full opacity positioned via `WITCH_MAP_BG_OFFSETS`. Otherwise the background color comes from the CSS custom property `--air-map-bg` (black, or `#160900` in witch mode).
...
Shown at the bottom of the strips window when a strip is selected **and** the aircraft is on the **approach channel** (`aircraft.controlSeat === CHANNEL_TYPE_APPROACH`, seat 5). Hidden for every other seat — including final approach under tower (seat 3) — and for any aircraft while `witchMode` is on. The window does not re-verify the mod — the **level-browser radar/strip gate** already checks/installs the plugin (BepInEx Debug Mode + `AC27Approach.dll`, MD5 vs the bundled Workshop / R2 release reference) before a radar/strip window can open, so `commandCapable` is treated as true here (the patch frames only reach the game through the AC27Approach plugin).

Command-line-style, mouse-only: the command builds up on ONE line (`CSN9355: Fly Heading 090`); at every step the next choices appear as a horizontal option row flush above the line (`Fly Heading | Fly Altitude | Fly Speed | Fly Waypoint | Clear for Approach | Cancel` → heading values `030…360 | Send | Add | Cancel` → `Send | Cancel`). Send appears once ≥1 option is committed. Escape mirrors Cancel (pending value pick → previous menu; else reset the line). **Send/Cancel/Escape keep the strip selected** — the composer resets its own line (`resetCommand`), so the next command can be composed for the same aircraft without re-clicking the strip; selection is released by clicking the window background (`handleBodyClick`).
...
- `Fly Altitude` — climb/descend-and-maintain: opens the same slider-panel pattern, a 1000-ft range from `ALT_MIN_FT` (1000) up to `max(ALT_MAX_FT (9000), current rounded to the nearest 1000)` — the thumb defaults to the rounded current (3300 ft → 3000) so Send always has a value; hidden while the aircraft has no live telemetry (`altitudeBase` memo — `position.y` in GU → ft via `FT_PER_GU = 100/0.3048` ≈ 328.084; 15.24 GU = 5000 ft). Sends `altitude` with `targetFt` (the picked value) and `rate: ALT_RATE_FPM` (1000 ft/min of game time) — the plugin moves the aircraft's **Y only** smoothly (same game-time scaling + pause rule as the turn); direction is implicit in the picked target. **China meter mode:** `Z*` ICAO airports (`isChinaIcao` in `patchCommands.js` — ICAO starting with `Z`, the same convention `flightDefaults.js:24` uses for the 'zh' language) switch the slider to a fixed 300–2700 m range (`ALT_MIN_M`/`ALT_MAX_M`) in 300-m steps (`ALT_STEP_M`), thumb at the rounded current clamped into range (`Math.round(altFt × 0.3048)`), readout zero-padded to constant width ("Fly Altitude 0900m"). The picked meters are converted back via `FT_PER_METER` (≈3.28084 — the constant `voiceTranscriptParser` already uses for "米"/"meters"/"m" altitudes) **before** `buildAltitudePayload`, so `targetFt` on the wire is always feet — the plugin contract is unchanged.
- `Fly Speed` — speed-and-maintain: opens the same slider-panel pattern, a 180–240 kt range thumb-defaulted to the live speed (hidden without telemetry; `speedBase` memo from `airSpeedKnot`). Sends `update_speed` with `kts` (raw knots — the plugin's default 1.5 kt/s of GAME time ramp applies; no `accel=N` from the UI, that's devtools-tuning only).
- `Fly Waypoint` — the UI sibling of the voice pipeline's "fly direct to X": picking it swaps the option row for the aircraft's current-approach waypoint list, **left to right in route order (entry → IAF)**, with the STAR name leading the row (`fcc-waypoint-star`). Target set (`waypointList` memo): the cache's per-STAR lists keyed `"STAR|runway"` (`starWaypoints` prop — fetched with the taxiways via `collect-values` `_starWaypoints`, which `electron/main.js` reads from `approachCache.starWaypoints`, built by `extractStarWaypoints()` in `src/acl/approach.js` from each STAR route's `AirwayNodes` $irefs resolved to named fixes), picked from the aircraft's own `ac.star`/`ac.runway` telemetry — with a "01" vs "1" zero-pad variant fallback, then the STAR's first runway entry, then the airport's full fix list (`waypoints` prop = `_airwayNodes`, the voice pipeline's direct-to target set). **Only names the approach radar actually displays survive — the same `FIX_NAME_RE` (`/^[A-Z]{3,5}$/`, `src/utils/constants/aviation.js`) the radar's waypoint layer applies** (skips `JN213`-style numbered nodes, `TurnPoint19`, lowercase/unnamed). Hidden entirely when the aircraft lacks a telemetry position or no target set resolves. Clicking a fix commits it (highlighted `fcc-waypoint-active`); Send/Add dispatch **one `update_heading` frame at the bearing from the aircraft's LIVE position to the fix** (`bearingDegrees(fromX, fromZ, wp.x, wp.z)` recomputed at Send time — the aircraft prop refreshes every 200 ms) with the standard `rate: TURN_RATE_DEG_S` — the plugin steers toward the fix, byte-identical to the voice direct-to command. **Fly Waypoint OVERRIDES Fly Heading**: picking it drops a composed heading from the line (never sent) and the heading option blacks out while a waypoint is pending/chained; chainable with altitude/speed, and Clear for Approach supersedes it like everything else.
- `Clear for Approach` — hand a STAR (state 30) aircraft onto final approach (state 5). **Supersedes a composed chain** (heading/altitude/speed/waypoint): choosing it drops the whole line (never sent) and removes the Fly Heading / Fly Altitude / Fly Speed / Fly Waypoint options (which also supersede **each other** — one frame per Send); only the `clear_for_appr` frame goes out (no kts/appr fields — the UI no longer exposes the plugin's optional approach-speed support). The frame also carries the same `rate: TURN_RATE_DEG_S` (keyed field `rate=3` in the frame) — the **handoff turn is smooth too**: the plugin arms a phase-gated turn at the handoff (pass-through while the aircraft still flies the STAR, then rotates the nose onto the approach course at 3°/s of game time once the transition lands, dropping on convergence).
- `Send` — dispatches the whole composed **chain IN ORDER — N sequential frames via `electronAPI.sendPatchCommand` (awaited one by one)**, then resets the line; the strip stays selected (the composer stays mounted — `key={aircraft.callSign}` only remounts when the selection changes to a different aircraft).

**CSS:** `FlightStripsWindow.css` — `.flight-strips-command-wrap` (position: relative, z-index 30), `.fcc-suggest` (absolute, `bottom: 100%`, monospace option row), `.fcc-suggest-item` with the action colors: `.fcc-suggest-send` (green — dispatch is a go), `.fcc-suggest-add` (blue — chain the pending command), `.fcc-suggest-cancel` (red — abandon), plus hover states (`.fcc-cmd` the line being built). Waypoint picker: `.fcc-waypoint-row` (scrollable when wide), `.fcc-waypoint-star` (muted uppercase STAR name leading the row), `.fcc-waypoint-btn` (min-width 44px, tabular-nums), `.fcc-waypoint-active` (green highlight on the picked fix). Slider panels: `.fcc-heading-slider` (plain `::-webkit-slider-thumb` circle — reused by Fly Altitude/Fly Speed) + `.fcc-heading-slider.fcc-plane-thumb` (plane-knob variant: 18×18, no border/radius, accent background, `-webkit-mask-image: var(--thumb-plane, none)` with `contain` sizing, `filter: drop-shadow`, `transform: rotate(var(--hdg, 0deg))` — `--hdg`/`--thumb-plane` set inline per element). Hidden with the command bar in witch mode via the existing `.flight-strips.witch-mode .flight-strips-command-bar` rule.
...
**Translation table** (EN + ZH, in `voiceTranscriptParser.js`): `fly/turn (left|right) (to) heading N` → update_heading (absolute; direction word tolerated); `climb|descend and maintain N` / `climb|descend to N` / `fly altitude N` / `level (off) (at) N` / `flight level N` → altitude; `reduce (speed) to N` / `increase speed to N` / `slow (down) to N` / `fly speed N` / `N knots` → update_speed; `clear(ed) (for) (the) [ils|rnav|visual|loc|vor|ndb] approach|appr` → clear_for_appr (supersedes a chain; a trailing `runway N (left|right|center)` designator is consumed and ignored — the aircraft's assigned runway stays authoritative); `fly direct to X` / `direct to X` / `direct X` / `直飞(向|至) X` → **update_heading toward the waypoint** (bearing from the aircraft's live UDP position to the fix via `bearingDegrees` — one frame, the plugin steers; X = the airport's `_airwayNodes` names, exact-first then D-L ≤ 2, or a spelled letter sequence: letter names, NATO words, bare letters, "double you"); bare `maintain/保持 N` disambiguated: unit word wins, then N ≥ 1000 → altitude else speed. Numbers: digit-by-digit, magnitudes (`two thousand`→2000, `九千`→9000), slots (`one eighty`→180), tens+ones (`thirty four`→34/304 — aircraft list disambiguates), `o`/`oh` for zero (speech engines render "oh" as "o"), FL ×100, Arabic fallback. Leading fillers (`um`/`uh`/`okay`/`sir`…) are stripped before the airline and number ("okay delta uh 3401" parses; "Okay Airways" still matches via the original-text-first rule). Unmatched text → `unsupported:` notices, never silently dropped. Parse failures return `reason` naming the failing stage (no airline / candidates not in list + first unparsed token / no aircraft data).

**Fuzzy matching (EN only, post-processing safety net over the vosk grammar):** exact-first, then Damerau-Levenshtein (OSA) with per-slot caps — pattern/airline words ≤1 (≤5 chars) or ≤2 (≥6), number/unit words and approach types flat ≤1 — plus a curated multi-token table for spelled-out forms ("r nav"→rnav, "eye el ess"→ils, "eff el"→fl). At most ONE deviation per pattern match (one connector-skip — "climb and then maintain 9000" — or one fuzzy word, never both). The cfa head (`clear(ed) (for) (the) [type] approach`) is the same budget: free skips of `for`/`the`, connectors/fillers and number words ("clear forty approach" → "for the" misheard, stray "oh"), plus one curated `foot`→`for` or fuzzy approach type; the required `approach|appr` tail never relaxes. Locked rules in `voiceFuzzy.js`: the two semantic inversions within distance (`ascend`→descend, `decrease`→increase) are **excluded**; fillers (`uh`/`um`/…) never fuzzy-map; 2-char tokens are exact-only except `to`/`an`/`on`/`of` (real SAPI artifacts); 3-letter airline codes are exact-only ("deal" never becomes dal — the phonetic stage re-checks this and blocks tokens within D-L 1 of a code, so "deal" can't reach "delta" either); command words are guarded out of flight-number scans ("CSC6918: right heading 120" must not eat "right" as a misheard digit) and the runway number scan guards `right`/`left`/`center` the same way ("runway three one right" parses 31R, never 318). **Phonetic skeleton stage:** when exact + letter-D-L both fail, a deterministic consonant-skeleton match (`enSkeleton`: th→θ, ch→x, silent gh dropped, vowels/glides dropped) runs against CLOSED sets only — single-word airline names ("cafe"→cathay, cf/cθ) and the 62 spoken forms of runway numbers 1–36 ("ethiopian right"→"three one"→31R). Guards: min skeleton 2, first symbol equal, raw-length diff ≤ 2, skeleton D-L exactly 1, unique best wins / ties fail. The **exhaustive acceptance table** (every dictionary word within cap of every grammar token, 1,500 accepts) is generated by `scripts/gen_voice_fuzzy_acceptance.mjs` from the real vocab tables into `tests/components/MapWindows/voiceFuzzyAcceptance.json`, round-trip-verified against the runtime lookups, and `--check` guards drift. **Callsign proximity fallback:** a resolved candidate one D-L edit from a live aircraft resolves anyway ("东方五拐八"→CES578 → CES5578) — approach-seat only (numeric `controlSeat !== 5` skipped), unique best wins, ties fail. Chinese stays exact-only except the proximity fallback (language-agnostic, after code resolution). `cafe`/`ethiopian` are parser-only tolerances — deliberately NOT grammar words. **Round 3:** callsign noise — `at` stripped before the flight number ("korean air at twenty twenty one"), `the` free-skipped INSIDE the number scan ("emirates for the eight thirty eight" → 4838; `at` deliberately still breaks mid-scan — it's a guarded pattern word), curated digit confusables `new`→two/nine (both readings stay candidates, the aircraft list disambiguates: "korean air new one" → KAL21 or KAL91; zero-candidate second chance only). Approach types: a spelled-letter stage after the curated windows ("eye oh ess" → i-o-s ≈ ils, D-L 1 — same letter tables as the waypoint slot; curated `eye el ess`/`i l s` still win first; `el`/`ess` alone < 3 letters stay rejected). Runway: plural suffixes (`rights`/`lefts`/`centers`) + slot-local confusables `through`→three, `urine`→one with a 2-token window tried first ("runway through urine right" → 31R). `i` is an EN connector ("and i clear for…"). Direct phrases accept a `flight` prefix (STT's render of "fly") and the curated mishearing `the wreck` → direct. **Airline code fix:** `'Korean Air': 'KAL'` (was AAR — Asiana's code; the map otherwise uses real ICAO codes) in `src/utils/constants/airlines.js` — spoken names/candidates derive from the map, parser unchanged. **Waypoint slot** (direct-to): its own rules — flat D-L ≤ 2 for every name (locked, per-airport pools are small: KDCA 99 names/23 D-L≤2 pairs, KJFK 55/4), exact always wins, ties first-wins; a token that IS a spoken letter form never takes the single-token fuzzy path (it's a spelling — "bee" must not degrade to a fix); spelled sequences resolve longest-first (names are 3–5 letters).

**Source files:**

| File | Purpose |
|------|---------|
| `voiceTranscriptParser.js` | **The core** — `parseVoiceTranscript(transcript, aircraftList, waypoints = [])` → `{ ok, callsign, aircraft, lang, commands: [{type, label, payload}], notices, renderedLine, reason? /* non-empty on ok:false */ }` (payloads ready for `sendPatchCommand`) + `parseVoiceCandidates(texts, aircraftList, waypoints = [])` (primary-then-alternates — first candidate yielding commands wins, selection-only never wins, no winner → primary unchanged) + `buildSyntheticAircraftList()` (CLI callsign resolution) + `matchWaypointValue(rest, waypoints)` (the direct-to slot: single-token exact → D-L ≤ 2 → spelled-letter sequence, consumed token count). Pattern matching is fuzzy-tolerant (F3: one connector-skip or one fuzzy word per pattern); the cfa head has the same one-deviation budget with free `for`/`the`/connector/number-word skips and a spelled-letter stage ("eye oh ess" → ils, round 3), and the runway slot parses with a suffix guard (`EN_RUNWAY_NUMBER_GUARD`), plural-suffix tolerance, a phonetic fallback over the 1–36 spoken forms ("ethiopian right"→31R) and slot-local confusables (`through`→three, `urine`→one, 2-token window first — "runway through urine right"→31R). `direct` patterns additionally accept a `flight` prefix + the curated "the wreck" mishearing; `i` is a connector. The `direct` type's payload is `update_heading` with `bearingDegrees(ac.position, waypoint)` — payload.type stays `update_heading` (wire contract). Pure, DOM-free, Node-loadable (explicit `.js` specifiers). Shared by the hook AND `scripts/voice_sim.mjs`. |
| `voiceNumberParser.js` | Spoken numbers: `parseEnglishFlightNumber()` / `parseChineseFlightNumber()` (digit candidates) + `parseSpokenNumberValue(tokens|string, lang, fuzzyGuard?)` (command values — digits/magnitude/slots/ZH positional, e.g. 幺二洞→120, 一百八→180). Accepts literal Arabic digits + punctuation ("6918:"). Fuzzy D-L ≤1 on number/unit words (`lookupEnNumberToken`, `lookupUnitWord` — "tree"→three, "to"→two, "nots"→knots) with fillers excluded; command words guarded out of flight-number scans, and an optional per-call `fuzzyGuard` Set blocks the fuzzy fallback (the runway scan passes `right`/`left`/`center`). Round 3: `the` is free-skipped (position-aware consumed) inside flight-number scans, and a zero-candidate second chance substitutes the curated digit confusables `new`→two/nine. Exports `EN_UNIT_WORDS`/`ZH_UNIT_WORDS`, `EN_NUMBER_WORD_KEYS` (cfa free-skip set) + the key lists (`EN_NUMBER_FUZZY_KEYS` etc.) for the acceptance generator. |
| `voiceCallsignParser.js` | `detectLanguage()` (CJK check), `parseCallsign()` — longest-match airline prefix (`getSpokenToCode()` from `AIRLINE_CODE_MAP` — `Korean Air`→`KAL` since) + number parsing + aircraft lookup; separate EN/ZH paths. EN airline names are fuzzy (D-L ≤1, ≤1 fuzzy word per name — "hainann"→hainan, "untied"→united; 3-letter codes exact-only) with a final phonetic skeleton stage ("cafe"→cathay; blocked for heavy-prefixes and tokens within D-L 1 of a code). Pre-number `at` is stripped as noise ("korean air at twenty twenty one"). A D-L ≤1 callsign **proximity fallback** against the live aircraft list fires when candidates exist but none matched exactly (approach-seat only, ties fail). Exports `getSpokenToCode()`, `matchPrefix()`, `matchPrefixFuzzy()`, `getSpokenNameWords()`, `callsignCandidates()` (all plausible callsigns for the CLI — fuzzy + skeleton-aware so the sim mirrors the app). |
| `voiceFuzzy.js` | **Fuzzy policy leaf** (imports nothing) — `damerauLevenshtein()` (OSA), `maxDistForWord()` (≤5→1, ≥6→2), `fuzzyMatch`/`fuzzyLookupKey` (exact-first, deterministic, per-candidate caps), `NON_FUZZY_WORDS` (fillers never fuzzy-map — also re-exported as `EN_FILLER_WORDS`), `SHORT_FUZZY_TOKENS` (`to`/`an`/`on`/`of`), `CURATED_EXCLUDE` (ascend→descend, decrease→increase blocked), `CURATED_CONFUSABLES` + `resolveCuratedPhrase()` (spelled-out forms; `direct: ['the wreck']` is a pure-mishearing entry), `FLIGHT_NUMBER_FUZZY_GUARD` (command words can't be swallowed as digits), `enSkeleton` + `skeletonMatch` (phonetic stage — closed-set only, guards: min skeleton 2, first symbol equal, raw-length diff ≤ 2, skeleton D-L exactly 1, unique best wins/ties fail), `EN_LETTER_WORDS` + `LETTER_WORD_TO_LETTER` + `TWO_TOKEN_LETTERS` (the waypoint slot's spelled-letter vocabulary: NATO word + letter name per letter in the grammar, parser-only variants like `el`/`see`/bare letters outside it). |
| `voiceCommandMatcher.js` | **Not used by the pipeline** (legacy fuzzy matcher for the native `CMD_*` vocabulary; its tests still pass). The active matcher is the greedy pattern engine inside `voiceTranscriptParser.js`. |
| `useVoiceCommands.js` | React hook — `useVoiceCommands(udpAircraft, waypoints = [])`; recognition session lifecycle (2s silence auto-stop **webkit only** — in Electron the worker's engine owns silence timing; 500ms cooldown). Electron: drives the offline vosk worker over IPC (`voice-stt-start`/`voice-stt-stop`, `voice-stt-event` pushes from the main process) — `voiceSttStart(waypoints)` sends the airport's waypoint names as the session's extra grammar words; browser fallback: `SpeechRecognition` (webkit). `processCandidates([primary, ...alternates])` → `parseVoiceCandidates(..., waypoints)` (logs `matchedFrom=primary|alternate#N`); `processTranscript(text)` is the single-candidate form for the browser path. Returns `{ listening, transcript, matchedCallsign, matchedCommand /* array */, confidence, error, voiceResult, matchedAircraft, isSupported, startListening, stopListening }`. |
| `electron/voice-stt-vosk.js` | The worker child — CJS, runs under ELECTRON_RUN_AS_NODE. JSON-lines protocol (in `{cmd:start, extraWords?: [...]}` / `{cmd:stop}` / `{cmd:exit}` — `extraWords` = the airport's waypoint names, validated `/^[a-z]{3,5}$/` and merged into BOTH recognizers so mixed "直飞 BELTT" utterances decode on the zh side too; out `ready`/`started`/`stopped`/`result{text,confidence,language}`/`detected`/`rejected{busy|low-confidence}`/`error{code}`). Session semantics: fresh Recognizers per `start`; `stop` flags — the in-flight phrase ALWAYS finalizes (vosk utterance boundary, or a 1.5 s silence grace after `stop` that resets only while speech flows — sox streams continuous silence, so an audio-based keep-alive would never fire) and its result is delivered before `stopped`; a `start` while finalizing resumes the session (re-emits `started`), while actively recognizing → `rejected busy`. Dual-lang: phrase results per recognizer are ACCUMULATED (finalResult() resets a recognizer — a boundary on one must not force the other mid-phrase), the winner is the higher mean word confidence (`result[]` word confs — not `words`; 0.3.39 naming), zh needs ≥3 chars (junk guard on EN speech) and is emitted space-free. Errors: `NO_MODEL`/`MODEL_LOAD_FAILED`/`NO_GRAMMAR` (boot), `SOX_NOT_FOUND`/`NO_AUDIO_DEVICE` (session). CLI: `--wav <file> [--extra w1,w2,...]` (16 kHz mono 16-bit; collects per-recognizer phrases at each own boundary, joins, single winner result), `--test` (model+grammar+sox self-check). |
...
| `electron/voice-grammar.json` | The Vosk `setGrammar` lists (`{words, wordsZh}`) — generated from the LIVE parser tables (EN_PATTERNS incl. the direct-to patterns, CFA sets, EN_NUMBER_KEYS, EN_UNIT_WORDS, airline words, fillers, phraseology literals, ZH_PATTERNS incl. 直飞(向|至), ZH_DIGIT, ZH_UNIT_WORDS, zh airline short forms, and the NATO + letter-name pairs from `EN_LETTER_WORDS` — `forms.slice(0, 2)`, 52 words; waypoint names are NOT committed — they're per-airport session extras) by `scripts/gen-vosk-grammar.mjs`; committed, deterministic, pinned by `voiceGrammarConsistency.test.js` (any parser-table edit without regeneration fails `npm test`). |
...
| `VoicePTTButton.jsx` | Hold-to-talk mic button with **pointer capture**: `onPointerDown` fires `onPress` and calls `setPointerCapture`, so the release is driven by `onPointerUp`/`onPointerCancel`/`onLostPointerCapture` (plus keyboard Space/Enter hold and a window-`blur` safety) — **never** by `onMouseLeave` (the old handler released whenever the cursor drifted off the 26px button). Idempotent press/release guards absorb duplicate events. States: idle (gray `IoMicOutline`), listening (red `IoMic` + pulse), matched (green flash 300ms), error (dimmed red). `feedback` prop shows the transient result line as tooltip. `witchMode` renders `witch/voice.png`. |
| `PttShortcutModal.jsx` | Remap modal for the global PTT hotkey (gear button right of the mic, greyed while PTT is unavailable): title + live key echo only — auto-focused capture box (Escape/X/backdrop close, Backspace disables, release settles to the saved value, failure flashes red with a `[PTT-SHORTCUT]` main-log line). The `mod-prompt-box` class restores the app UI font under the strips 10px monospace root. |
| `electron/pttShortcut.js` | Main-process global-hotkey manager (CJS, deps injected — no `require('electron')` — unit-tested in `tests/electron/ptt-shortcut.test.js`): `createPttShortcutManager({globalShortcut, loadConfig, saveConfig, getStripsWindows, onTrigger, onPress, onRelease, keyWatch})` — register/unregister/dispose, `get`/`set` backing (`pttShortcut` in `ac27-config.json`), focused-first strips routing. With a `keyWatch` it is **hold-to-talk** (broadcast `global-ptt-down` on press + arm the watcher, `global-ptt-up` on the key-up edge, absorbing auto-repeat); otherwise it falls back to `global-ptt-toggle`. Wired in `electron/main.js` (`get-ptt-shortcut`/`set-ptt-shortcut` IPC, registered at `whenReady`, `unregisterAll` at `will-quit`) + `electron/preload.js` (`getPttShortcut`/`setPttShortcut`/`onGlobalPttDown`/`onGlobalPttUp`/`onGlobalPttToggle`). |
| `electron/pttKeyWatch.js` | Windows key-UP watcher for hold-to-talk (koffi → `user32!GetAsyncKeyState`, loaded lazily, Windows-only; `acceleratorToVk` maps an Electron accelerator to its main virtual-key). `createPttKeyWatch({isDown, intervalMs})` → `{isAvailable, watch(vk, onUp), cancel}` — polls at 40 ms, fires `onUp` once on the down→up transition; re-arming replaces a pending hold (auto-repeat). Unit-tested in `tests/electron/ptt-key-watch.test.js`. |
| `src/utils/pttShortcut.js` | Pure accelerator helpers shared by the modal (unit-tested in `tests/unit/ptt-shortcut.test.js`): `PTT_SHORTCUT_DEFAULT` (`Shift+Space`), `normalizeAccelerator`, `isValidAccelerator` (Modifier+Key or bare F-key — bare letters/Space rejected, Windows can't register them), `buildAcceleratorFromEvent` (modifier-only → `''` wait, Tab/F-keys pass, unusable keys → null). |

**Integration points in `FlightStripsWindow.jsx`:**
- Waypoints: the collectValues effect (taxiways) also captures `_airwayNodes` → `useVoiceCommands(udpAircraft, waypoints)` — no extra IPC
...
- Global hotkey: the `onGlobalPttToggle` subscription (stale-closure-safe refs so the single listener never goes stale) toggles `handleVoiceStart` / `voice.stopListening`; the gear button (`IoSettingsOutline`, `ptt_shortcut_update` tooltip) sits right of the mic with `ptt-gear-disabled` while `commandCapable !== true`; the window-level Escape handler also closes the shortcut modal

**Audio-free testing:** `node scripts/voice_sim.mjs "CSC6918: climb and maintain 9000, reduce speed to 180 knots" [--live] [--alternates "a|b"] [--waypoints file.json]` — runs the exact pipeline (`parseVoiceCandidates`), prints the command-window line + payload JSON, and with `--live` sends the 0x00E7 frames to the game at 127.0.0.1:20267 (send-only socket; never binds 20266). `--aircraft file.json` exercises the seat gate; `--waypoints file.json` provides the direct-to target set (`[{name,x,z}]` — same shape as `_airwayNodes`); `--alternates` mirrors the worker's alternate hypotheses (first candidate yielding commands wins, prints `matched from:`); dry-run is the default. Frame builder shared via `electron/patchFrame.js`. **Fuzzy acceptance table:** `node scripts/gen_voice_fuzzy_acceptance.mjs` regenerates `tests/components/MapWindows/voiceFuzzyAcceptance.json` from the real vocab tables (needs network for the 370k-word list; `--check` diffs without writing). **Grammar regeneration:** `node scripts/gen-vosk-grammar.mjs` (or `--check`); **model fetch:** `node scripts/fetch-vosk-model.mjs` (en LARGE `vosk-model-en-us-0.22` ~1.9 GB + zh small `vosk-model-small-cn-0.22` ~42 MB — the exact pair the voice build ships; `--check` verifies presence). **STT round-trip (no mic):** `node scripts/voice-stt-test.mjs` — synthesizes EN + ZH phrases via System.Speech **TTS** (16 kHz mono WAVs; SKIPs a language without an installed voice), runs the worker `--wav`, asserts ready → result → stopped + plausible tokens per language.
...
**UX notes:** vosk emits **final results only** (one `result` per phrase — no interim text), so the "Heard: …" tooltip updates per phrase; the first press after launch takes a few seconds to ~20 s (the LARGE en model loads on first use — the status probe absorbs it), later presses are instant. Multiple commands within one hold each produce a result. **The PTT release can never discard a phrase** — the child finalizes the phrase in flight (vosk boundary or 1.5 s silence grace) before the session ends, and a fast re-press continues the same session. A result therefore lands up to ~1.5 s after you stop speaking; if none appears, the main-process log distinguishes the failure: `detected` + `rejected` lines = the mic heard audio but it couldn't be parsed; nothing at all = the mic heard nothing (check the Windows default recording device — sox only uses the default). A worker crash mid-hold surfaces a visible error (button tooltip). **Model paths / env overrides** (child-side, env-first): en model `VOSK_MODEL_DIR` → `<VOICE_RESOURCES>/models/vosk-model-en-us-0.22`; zh model `VOSK_ZH_MODEL_DIR` → `<VOICE_RESOURCES>/models/vosk-model-small-cn-0.22`; sox `VOSK_SOX_PATH` → `resources/sox/sox.exe` (packaged) → `bin/sox/sox.exe` (dev); vosk DLLs `VOSK_LIB_DIR` → `resources/vosk` → `bin/vosk`.

**Tests:** the voice Vitest suites (`voiceNumberParser`, `voiceCallsignParser`, `voiceCommandMatcher` legacy, `voiceSpokenNumberValue`, `voiceTranscriptParser`, `voiceDeviationMatrix` — sections 6b/8c = cfa budget + runway golden-path + proximity/skeleton rows (+ round-3 rows: letter-spelled ILS, callsign noise at/the/new, KAL code, runway through/urine, flight-prefix direct), section 12 = direct-to rows incl. payload exactness, `voiceWaypointMatcher` — slot unit tests, `voiceFuzzy`, `voiceSkeleton` — enSkeleton/skeletonMatch guards + runway integration, `voiceCandidates` — waypoint threading, `voiceFuzzyAcceptance`) are unchanged, plus `voiceGrammarConsistency.test.js` (pins `voice-grammar.json` ⇄ live parser tables, en + zh). End-to-end gates are the script harnesses above (drain test, STT round-trip, vocab check). Global-hotkey suites: `tests/components/MapWindows/PttShortcutModal.test.jsx` (15 — load/echo, capture+save, Backspace-disable, Escape/X/backdrop close, register-failure flash + log, in-flight-save guard, missing-bridge/`onClose` tolerance), `tests/unit/ptt-shortcut.test.js` (4 — default, normalization, validity, event building), `tests/electron/ptt-shortcut.test.js` (20 — config defaults, set/persist/swap/disable, register/save failure paths, focused-first routing, dispose, throwing-host tolerance).

### AC27Approach Plugin Install/Update — browser gate (was: Flight Strips prompt)

Installing/updating the AC27Approach plugin (the DLL that makes the composer + voice PTT work) is now handled by the **level-browser radar/strip gate** (`BrowserScreen.jsx`), not the Flight Strips window. Clicking a Surface Radar / Approach Radar / Flight Strips toggle runs a one-shot flow:

- **Platform:** the three toggles render only when `get-system-info.platform === 'win32'` (and not in demo mode). The same Windows gate (`isWindows`) hides the settings-menu **Debug Mode** item (`BrowserScreen.jsx`) and the matching help-overlay rows (`BrowserHelpOverlay` `isWindows` prop — the `cards` section and the Debug Mode settings row are `modOnly`). BepInEx is a win-x64 IL2CPP runtime, so mac/linux builds show neither the Debug Mode entry nor the toggles.
- **Warning:** a modal warns that the feature mods the game ("This feature require modding the game…") with a "do not show again" checkbox persisted as the cache flag `radarModWarnDismissed` (new key → `CACHE_VERSION` bumped). With Debug Mode off the dialog adds "Shut down the game and enable debug mode in setting." and never installs; with Debug Mode on, agreeing installs.
- **MD5 check:** `checkCommandCapability()` compares the deployed DLL's MD5 against the release reference — the **bundled Workshop DLL** on a Steam/Workshop build (`resolveWorkshopBundledDllPath`), the **R2 object** (ETag = build MD5) on a normal/voice build. In place + matching → the window opens with no install. Missing, or a packaged mismatch → `downloadApproachDll()` (R2 on normal builds, the bundled path on Steam/Workshop — never R2) then `installApproachDll()`, with a file-picker fallback. Dev (`!app.isPackaged`) never enforces the MD5, so `npm start` / `npm start steam` don't nag.

The Flight Strips window therefore **does not run `checkCommandCapability` again** and has no install/update overlay: `commandCapable` is treated as true (the browser already guaranteed the mod). The legacy `ApproachPluginInstallOverlay` / `handleLoadDll` / `STORAGE_KEY_APPROACH_PROMPT` prompt code remains in `FlightStripsWindow.jsx` but is unreachable.
...
- **Airport card background:** Each airport card renders a mini ground-radar SVG via `AirportCardMap` component (same geometry data as GroundMapWindow: area polygons, taxiway paths, runway rectangles). The SVG is oversized (`cardHeight / 0.30`) and centered behind the card; `overflow: hidden` clips it to card bounds so the card acts as a window showing ~30% of the total background. ViewBox aspect ratio is forced to match the card's (984 / cardHeight) so `preserveAspectRatio="slice"` has no distortion. Card height is computed from row count: `HEADER_H + numRows × ROW_H` (46 + n×35 px); collapsed cards pass `numRows={0}`.
- **Toggle buttons (ENABLED):** the per-card map toggle buttons (Surface Radar `toolbar_surface_radar`, Approach Radar `toolbar_approach_radar`, Flight Strips `toolbar_flight_strips`) are gated behind `BROWSER_RADAR_TOGGLES_ENABLED = true` (exported from `BrowserHelpOverlay.jsx`, imported by `BrowserScreen.jsx`); the matching "Airport" help section renders too. They show only when NOT in demo mode (`!isDemo`), carry an `.active` class when the corresponding window is open for that airport, and `stopPropagation` so clicking them never collapses the card. Flip the constant to `false` to hide them again.
...
- **Collapsible cards + session auto-collapse:** the whole `.airport-card-header` is a click/keyboard toggle (`role="button"`, `aria-expanded`, chevron: `IoChevronDown` when open / `IoChevronForward` when collapsed) that shows or hides the card's `.level-row`s (when the radar toggles are re-enabled they `stopPropagation` so they never collapse). When collapsed the `AirportCardMap` receives `numRows={0}` (background shrinks to header height). A one-shot **auto-collapse fit pass** (`useLayoutEffect` in `BrowserScreen.jsx`) measures each card's real `offsetHeight`/header height and, for airports the user hasn't explicitly toggled, collapses trailing cards (last first) until the summed heights fit `.browser-content`. It runs **once per app session** (`browserAutoCollapseDone` in the store) and never re-runs on resize, navigation, or user toggles. The resulting state is session-persisted (`browserCollapsedAirports` = explicit user prefs, `browserAutoCollapsed` = fit result) so navigating into a level and back restores the chosen layout; `setRootPath` clears all three.
...
const helpTip = useCallback((key) => t(key).replace(/^\{\{btn:\w+\}\}\s*[—–\-ï¼š:]\s*/, ''), [t]);
...
- **`AIR_MAP_BG_OFFSETS`** (in `src/utils/constants/map-config.js`): The old per-airport approach-radar background-image config (`dx`/`dy`/`w`/`bg`/`bgUnder`, `STAR_BG_OFFSETS` renamed). **Not consumed by the current UI** — the export remains for reference only. Witch mode still uses `WITCH_MAP_BG_OFFSETS`.
...
- **`GROUND_MAP_CENTER_OFFSET`** (`src/utils/constants.js`): Per-airport viewBox center offset in game units (`{x, z}`). Used by GroundMapWindow to fine-tune initial camera position. Entries for ZSJN, KJFK, and ZGSZ.
