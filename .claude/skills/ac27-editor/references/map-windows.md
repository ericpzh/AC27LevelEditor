...
Push-to-talk voice command system for the Flight Strips window. Speech backend (2026-08-05): **Windows System.Speech** (`SpeechRecognitionEngine`) driven by a PowerShell 5.1 worker (`electron/voice-stt.ps1`, spawned by `electron/voiceSttWorker.js` in the main process, JSON-lines protocol over stdin/stdout, `powershell.exe` guaranteed on Windows — `System.Speech` is NOT in PowerShell 7+). The original **Web Speech API** was removed because Chromium streams mic audio as a chunked HTTPS upload to Google's speech API, which is shut down for shell environments: it works in Chrome but fails with a `network` error in Electron (console: `chunked_data_pipe_upload_data_stream … OnSizeReceived failed with Error: -2`), and the hook used to swallow it silently. `electron-voice` was also evaluated and rejected (no license, requires Rust + Neon + Vosk native modules). The webkit path survives only as a **browser-mode fallback** (the hook runs it when `window.electronAPI` is absent — plain vite in Chrome). Retargeted from the old native `CMD_*` command IDs to the **patch-command vocabulary** (2026-08-05): the voice chain dispatches `sendPatchCommand` payloads byte-identical to what `FlightPatchCommandBar` composes.
...
| `useVoiceCommands.js` | React hook — recognition session lifecycle (2s silence auto-stop **webkit only** — in Electron the worker's engine owns silence timing; 500ms cooldown). Electron: drives the System.Speech worker over IPC (`voice-stt-start`/`voice-stt-stop`, `voice-stt-event` pushes from the main process); browser fallback: `SpeechRecognition` (webkit). `processCandidates([primary, ...alternates])` → `parseVoiceCandidates` (logs `matchedFrom=primary|alternate#N`); `processTranscript(text)` is the single-candidate form for the browser path. Returns `{ listening, transcript, matchedCallsign, matchedCommand /* array */, confidence, error, voiceResult, matchedAircraft, isSupported, startListening, stopListening }`. |
| `electron/voice-stt.ps1` | PowerShell 5.1 worker — `SpeechRecognitionEngine` (en-US preferred, zh-CN fallback; UTF-8 JSON lines via `[Console]::Out`, never Write-Host), dictation grammar, **single-threaded tick loop**: synchronous `Recognize()` (results come from the return value — the async event path is fragile in PS 5.1 and crashed in testing), `EndSilenceTimeout` 1s / `InitialSilenceTimeout` 2s, stdin read async via `BeginRead`/`EndRead` (no threads). `result` lines carry `alternates` (top ≤5 non-empty deduped alternate phrases, excluding the primary, confidence-ordered — `Get-Alternates`/`Emit-Result`). `stop` flags the session; the phrase in flight always finalizes and its result is delivered before `stopped`. Diagnostic events: `detected` (mic audio crossed the threshold) and `rejected` (low-confidence / busy). CLI modes: `-Wav <file>` (wave file), `-Mic [-MicSeconds N]` (default audio device), `-TestWav <file>` (test-only: wav stands in for the mic in interactive mode). |
| `electron/voiceSttWorker.js` | Main-process bridge — spawns `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File voice-stt.ps1`, state machine `idle→starting→ready→recognizing→ready`, **release-drain** (`STOP_DRAIN_MS` 1500): the `stop` command is delayed so the ps1 can finalize the in-flight phrase; a re-press inside the window cancels the pending stop (seamless continuation, no restart), a re-press after it is forwarded and the ps1 continues the finalizing session. Request-scoped event routing to the initiating window, `will-quit` dispose (3s kill grace — the ps1 only reads stdin between recognition ticks). |
...
**Audio-free testing:** `node scripts/voice_sim.mjs "CSC6918: climb and maintain 9000, reduce speed to 180 knots" [--live] [--alternates "a|b"]` — runs the exact pipeline (`parseVoiceCandidates`), prints the command-window line + payload JSON, and with `--live` sends the 0x00E7 frames to the game at 127.0.0.1:20267 (send-only socket; never binds 20266). `--aircraft file.json` exercises the seat gate; `--alternates` mirrors the worker's alternate hypotheses (first candidate yielding commands wins, prints `matched from:`); dry-run is the default. Frame builder shared via `electron/patchFrame.js`. **Fuzzy acceptance table:** `node scripts/gen_voice_fuzzy_acceptance.mjs` regenerates `tests/components/MapWindows/voiceFuzzyAcceptance.json` from the real vocab tables (needs network for the 370k-word list; `--check` diffs without writing). **STT link (no mic):** `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File electron/voice-stt.ps1 -Wav sample.wav` — runs the same worker protocol against a wave file (emits `ready` → `result` (with `alternates`) → `stopped` lines, exit 0). **Mic check (no app):** `… -File electron/voice-stt.ps1 -Mic` — listens on the default audio device for 60s (`-MicSeconds N` to change), emitting `result` lines as you speak; verifies the mic path standalone (also tells you WHICH device SAPI hears — `SetInputToDefaultAudioDevice`). **Worker lifecycle regression:** `node scripts/voice-drain-test.mjs` — start/release/re-press/dispose cycle against the real worker.

**Mic ownership:** the **PowerShell worker owns the mic** (its `SetInputToDefaultAudioDevice()` — Windows microphone privacy settings apply to powershell.exe, not the renderer). The renderer never requests `media` in Electron, and `main.js`'s permission handler now denies everything. Only the browser-mode fallback (vite in Chrome) uses the browser's own mic permission, which is why `index.html`'s CSP still has `media-src 'self' mediastream:`.

**UX notes:** System.Speech emits **final results only** (one `result` per phrase — no interim text), so the "Heard: …" tooltip updates per phrase; the first press after launch takes ~1–2 s (worker warm-up), later presses are instant. Multiple commands within one hold each produce a result. **The PTT release can never discard a phrase** — the engine finalizes the phrase in flight (1s silence) before the session ends, and a fast re-press continues the same session. A result therefore lands up to ~1s after you stop speaking; if none appears, the main-process log distinguishes the failure: `sound detected` + `rejected` lines = the mic heard audio but it couldn't be parsed; nothing at all = the mic heard nothing (check Windows default recording device — SAPI only uses the default). A worker crash mid-hold surfaces a visible error (button tooltip) instead of the old silent failure.

**Tests:** 466 Vitest tests across 9 files: `voiceNumberParser.test.js` (41), `voiceCallsignParser.test.js` (50), `voiceCommandMatcher.test.js` (21, legacy), `voiceSpokenNumberValue.test.js` (32), `voiceTranscriptParser.test.js` (53), `voiceDeviationMatrix.test.js` (212 — data-driven rows from the deviation matrix, incl. the fuzzy-acceptance section), `voiceFuzzy.test.js` (31 — D-L/fuzzyMatch/exclusion primitives), `voiceCandidates.test.js` (10 — parseVoiceCandidates ordering), `voiceFuzzyAcceptance.test.js` (16 — round-trips every entry of the generated acceptance table against the runtime lookups).
