...
Shown at the bottom of the strips window when a strip is selected **and** the aircraft is on the **approach channel** (`aircraft.controlSeat === CHANNEL_TYPE_APPROACH`, seat 5). Hidden for every other seat — including final approach under tower (seat 3) — and for any aircraft while `witchMode` is on. Also gated on `checkCommandCapability` resolving to true (the three-way check: BepInEx Debug Mode installed AND the `AC27Approach.dll` deployed AND MD5-matching the latest release — the patch frames only reach the game through the AC27Approach plugin, which only exists under BepInEx); the gate re-checks on mount, aircraft change, and window focus — while unknown the composer stays hidden so the gate never flashes.
...
### AC27Approach Plugin Install/Update Prompt
...
- **Toggle buttons (HIDDEN, kept for revival):** the per-card map toggle buttons (Surface Radar `toolbar_surface_radar`, Approach Radar `toolbar_approach_radar`, Flight Strips `toolbar_flight_strips`) are gated behind `BROWSER_RADAR_TOGGLES_ENABLED = false` (exported from `BrowserHelpOverlay.jsx`, imported by `BrowserScreen.jsx`) and do not render in the level browser; the matching "Airport" help section is omitted too. Flip the constant to re-enable. When enabled, they show only when NOT in demo mode (`!isDemo`), carry an `.active` class when the corresponding window is open for that airport, and `stopPropagation` so clicking them never collapses the card.
- **Toggle handler:** Checks `openGroundRadarAirports` / `openAirRadarAirports` / `openFlightStripAirports` Sets — if ICAO present, calls `closeXxxMap` IPC; otherwise calls `openXxxMap` IPC. Updates zustand state on both paths. (Unused while the buttons are hidden, but left in place.)
