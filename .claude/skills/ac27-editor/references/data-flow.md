...
3. `init-airport-cache` IPC → loads audio clips + pre-scans approach data + dropdown values per airport:
 - Scans `.acl` files through `isCacheAclFile()` in `electron/main.js` — **excludes** `.acl.bak` backups and all variants matching the `HIDDEN_LEVEL_RE` regex (`tutorial`, `bench`, `test`, `crossrunway`, `dev`, `endless`, `.prod`) **UNLESS** the filename is whitelisted in `DEMO_VISIBLE_BASES` or `PROD_VISIBLE_BASES` — browser-whitelisted levels are ALWAYS scanned regardless of the blacklist regex so their geometry is still cached (a whitelisted level whose name matches the blacklist would otherwise lose its taxiway/stand/runway/area geometry and render blank radar windows). Demo slices (`.demo.acl`) and `_emerg` files are included. (Note: the browser visibility filter itself uses explicit whitelists — `PROD_VISIBLE_BASES` / `DEMO_VISIBLE_BASES` in `src/utils/constants/ui.js` — not a blacklist regex.)
 ...
 - Extracts `specDB` + `designatorMap` — **v5 primary is the global `StreamingAssets/aircraft_profiles.csv`** (`loadGlobalSpecDB` → `parseAircraftProfilesCsv`; same for all airports, cloned per airport; `specDB` indexed by both Designator and AircraftType). `extractSpecificationDB` scanning of `Specification` objects inside jetway/aircraft entries remains as fallback for pre-v5 fixtures. Also builds `appPointMap` ((STAR,Runway) → AppPointList, from PKStaticEntities runway Routes RouteType=1), `totalApproachTimes` (STAR → seconds, from route path lengths), both **merged across every `.acl` in the airport** (v5 per-level filtering — each level only carries a subset of STAR/SID; the cache merges with runway-dedup so ZSJN `leisure_1` RWY19 + `leisure_2` RWY01 both appear)
 - Extracts State=5 data: `state5ParamsMap` (runway → `{pathPointList, touchDownPosition, approachDirection, initialPosition, routeName?}`), `starPaths` (STAR → waypoint array), and STAR↔runway maps from PKStaticEntities `runway:*` entries' `Routes` (RouteType=0) — all **merged across every `.acl`** (v5 per-level filtering; `TouchDownPosition` via `PhysicalRunwayStaticItem` `$iref` indirection, see `acl-format.md`)
 - Extracts `runwayThresholds` (PhysicalName → threshold pair, from runway `ThresholdPoints` `$iref`s → taxiway-node positions; `PhysicalName` via nested `PhysicalRunwayStaticItem` inline/`$iref` + `_findPhysicalNameByIref` fallback + `"$id": N` raw-text scan for v5) for StarMap/MapWindow visualization — merged across all files
 - Extracts `taxiwayPaths` (taxiway centerline polylines from PKStaticEntities `taxiway-segment:*` entries via `taxiway.js`) — **merged from ALL `.acl` files** with coordinate-based dedup (`toFixed(2)` precision) — same merge the v5 STAR/SID paths now use. Used by GroundMapWindow.
 - Extracts fixes: `airwayNodes` (`airway-node` PK entities, ICAO-style only via `FIX_NAME_RE` `/^[A-Z]{3,5}$/`) — merged across all files (v5, deduped by `pk`)
 ...
 - Extracts APPR data: `apprPaths` (RNAV approach routes, RouteType=1 via `sid_goaround.js`'s `extractApprRunwayMappings`/`buildApprPaths`), `apprRunwayMap`/`runwayApprMap` — also merged across all files
  ...
  - `_buildCollectValuesBase` also injects **`_gameCallsigns`** — the install-global **flight-number pool**, the union of every airport's `_flightNums` (`src/utils/gameCallsigns.js:unionFlightNums` over `airportCache`, computed fresh). The game pre-records one clip per callsign and throws `InvalidOperationException: ... failed to allocate callsign '<CS>' for crew voice '<voice>'` for a number it has no recording of, **and** for a foreign airline (not in the airport's own set) even when that number is recorded. **`catalog.bin` is deliberately NOT used as the source:** an in-game stress test (all 585 catalog callsigns written across the 4 airports' levels) proved it over-reports — `KLM982`/`KLM983`/`KLM984` are listed but crash — and allocatability is also voice/bundle scoped (`CCA9052` is real but only allocates for a Chinese-captain voice). The shipped union is every-callsign-proven. So the Flight # picker unions `_flightNums[airline]` with `_gameCallsigns[airline]` (renderer `FlightTable.jsx`, validator `runTripleValidation`, MCP `buildConstraints`) — only the **number** is relaxed; `AirlineCode` and its whitelist stay per-airport. Never persisted to `cache.json`.
  ...
  - Caches in memory as `airportCache[icao] = { audioCallsigns, approachData, dropdownValues, runwayPairs, standPositions, areaData }` — `dropdownValues` contains no `Stand`/`Runway`/`Airway`; `standPositions` in the persisted shape is always `{}` (live map lives in `liveSceneryCache`).
...
1. `handleSave()` → `validateCallsigns()` → `runTripleValidation()`:
 ...
 - (e) STAR-required validation — arrival flights must carry a STAR (`val_star_required`; the game's `FlightPlan.Init()` drops STAR-less arrival legs at level load). Gated on `_starRunwayMap` being non-empty
 - (f) Duplicate registration validation — flags flights where the same Registration appears in multiple departure or arrival flights (see below)
...
4. `save-acl` IPC → sorts flights → looks up approach cache for the airport → generates full ACL via `_rebuildStaticDataSections()`:
 - **Game-compat normalization runs FIRST inside `_rebuildStaticDataSections`** (`_normalizeFlightsForGameCompat`, before any section navigation): auto-repairs the four fuzz-discovered init-rejection classes by mutating `flights` — (1) duplicate registrations (ARR+DEP sharing a reg) get the non-frame-linked side renamed to a fresh `reg-suffix`; (2) arrivals at stands whose docked departure doesn't depart in-scenario (takeoff past `Config.endTime`) or lands before the docked aircraft's off-block are MOVED to a safe stand (`GAME_STAND_MIN_GAP_SEC` = 20 min arrival separation; DEP→ARR handovers at any gap are allowed, ARR→DEP cross-reg is pre-gated); (3) safe stands drawn from the caller's `standPool` (renderer `sceneryMaps.standIdToGuid` keys) + frame jetway-key stands + stands in use; (4) **STAR-less arrivals get `Airway` filled** from `approachCache.runwayStarMap` (first STAR of the runway), and arrivals on departure-only runways (no STAR data) are moved to the first arrival-capable runway. Logs `[GAME-COMPAT]` lines per rename/move/star. See `tests/integration/save_gamecompat.test.js`.
 ...
 - **7c. Remove orphaned RuntimeEntities (`_removeOrphanedFlightEntities`):** Removes or renames `$k` entries (`flight-plan:REG`, `aircraft:REG`, `aircraft-animator:aircraft:REG`) whose registration no longer exists in rebuilt StaticItems. Handles rename via `renameMap` and falls back to `StaticItem.$fstrref` for corrupted saves. Updates `$rlength`.
  - **7d. Cleanup EventLog LatestEvents (`_cleanupEventLogLatestEvents`):** On every save, fully clears the `LatestEvents` dictionary inside `singleton:event-log.$v.LatestEvents` — sets `$rlength` to 0 and `$rcontent` to `[]`, preventing stale `aircraft:REG` entries from accumulating across saves.
 - **Centralized $iref remapping (`_IdMapper.remapIrefs`):** Union-find structure per segment. Applies old→new `$id` mappings collected during 7b-1/7b-3 to correct `$iref` references in preserved entries. **Before remapping, every `$id` owned by a KEPT (non-rebuilt) RuntimeEntities entry is excluded from the mapper** via `_collectKeptRuntimeEntityIds` (a string-aware structural scan of `$rcontent`, no regex) — jetway/radio-channel/singleton/other entries keep their ids, so a rebuilt object's oldId that collides with a kept id is never rewritten (e.g. an aircraft `$iref:615` pointing to a kept radio-channel stays 615).
 ...
 - Writes `.acl` + `.csv` — `writeAcl()` renumbers every segment's `$id`/`$iref` to strictly ascending text order (`src/acl/id_renumber.js`) before encoding; without it the game misbinds inline values (descending ids from construction-order allocation) → `SetDockingTarget(null)` NullReferenceException per docked jetway
...
The app uses a unified **`cache.json`** in `userData`. It contains `gameRoot`, `lang`, `cacheVersion`, `builtAt`, and `airports`, plus a top-level **`flags`** bag for persisted UI prefs (e.g. `flags.liveryModHintDismissed` from the post-save mod-enable prompt). Both cache rebuild paths (`init-airport-cache`, `refresh-root-scan`) carry `flags` over from the previous cache — never drop it.

Cache validity is determined by a standalone **`CACHE_VERSION`** constant (integer, hand-bumped in `src/utils/constants/timing.js`), NOT by `app.getVersion()`. This decouples cache invalidation from app updates.

**⚠️ CACHE_VERSION rule:** Any change to the shape of `cache.json` (new fields in the approach cache object, new top-level keys, changed structure of `approachData`, `fileTypeMaps`, etc.) MUST bump `CACHE_VERSION` in `src/utils/constants/timing.js`. The re-scan happens transparently during App.jsx boot — `initAirportCache()` detects the version mismatch and rebuilds the cache silently before BrowserScreen mounts. Without the bump, old cached data will silently corrupt saves. Examples of changes requiring a bump: adding `state5ParamsMap`, changing `fileTypeMaps` from per-airport to per-file, adding `.bak` files to the scan set, adding `taxiwayPaths`/`sidPaths`/`missedAppPaths`/`airwayNodes`/`starWaypoints` to `approachData`, removing `saveTimeOffsets`/`typeMap` from the serialized schema.
...
- `PROD_VISIBLE_BASES` (ordered array, **24 entries**) — the only levels shown in production (non-demo) mode; **array position = browser display order**:
 `ZSJN_leisure_1.acl`, `ZSJN_leisure_2.acl`, `ZSJN_runwaychange.acl`, `ZSJN_peakdeparture.acl`, `ZSJN_taixwayclosed.acl`, `ZSJN_surfaceradarinvisible.acl`, `KJFK_leisure_1.acl`, `KJFK_leisure_2.acl`, `KJFK_runwaychange.acl`, `KJFK_peakdeparture.acl`, `KJFK_peakarrival.acl`, `KJFK_surfaceradarinvisible.acl`, `KDCA_leisure_1.acl`, `KDCA_leisure_2.acl`, `KDCA_runwaychange.acl`, `KDCA_peakdeparture.acl`, `KDCA_peakarrival.acl`, `KDCA_surfaceradarinvisible.acl`, **`ZGSZ_leisure_1.acl`, `ZGSZ_leisure_2.acl`, `ZGSZ_runwaychange.acl`, `ZGSZ_peakdeparture.acl`, `ZGSZ_peakarrival.acl`**, `ZGSZ_surfaceradarinvisible.acl` (Surface Radar Invisible / 无场面雷达). The browser's production scope is **24** levels (`ZGSZ_Endless.acl` is intentionally NOT listed: it is a flightless scenery/endless level with no WorldState flights, so it can't be opened/saved in the editor).
...
**Display names:** Levels show localized names from `src/utils/i18n.js` under `level_name_<base-filename-without-.acl>` keys (e.g. `level_name_ZSJN_leisure_1` → "悠闲时刻" / "Relax Time"; `.demo` variants included, e.g. `'level_name_KJFK_peakarrival.demo'` — quoted because the key contains dots; `ZGSZ_*` keys: `level_name_ZGSZ_leisure_1`/`_leisure_2`/`_runwaychange`/`_peakdeparture`/`_peakarrival` → "Relax/Busy Time…", `*_surfaceradarinvisible` → "Surface Radar Invisible"/"无场面雷达" (a `level_name_ZGSZ_Endless` key also exists in the dictionary but is unused since that level is not in `PROD_VISIBLE_BASES`). BrowserScreen looks up `t('level_name_' + filename.replace(/\.acl$/i, ''))`; `t()` falls back to the raw key if missing.

**Row layout:** `[display name — large] [time range HH:MM-HH:MM] [small filename via stripSuffixes] [arrivals/departures stats] [arrow]`. The first column is fixed-width per language (`--tod-width`: zh 80px / en 130px, set inline on `#screen-browser`) so the name/time/filename sections align vertically across airport cards. `sortLevelRows` does not sort by time.
...
**Failed-parse levels:** Files that can't be opened (e.g. "No WorldState flight data", Git LFS stubs) are intentionally **kept** in the whitelist filtering — the level row itself renders nothing (`return null`), but the airport header and its radar-window toggles stay available for that airport.
...
- **Challenge Level display:** Files with `_emerg` in their name sort to the bottom of the browser list (`isEmer` flag exposed via `get-airport-files-info` IPC, checked in `BrowserScreen`); display names come from `level_name_*` i18n keys.
