...
`patchSceneryBlob(snapshotText, graph, blobTypeMap, meta)` → `newText` (no disk I/O):
...
3. **Lossless no-op:** if there are no removals, no new elements, no moved nodes, and no moved areas, `patchSceneryBlob` returns `snapshotText` unchanged — **but it still reconciles the checkpoint frame's physical-runway AND jetway runtime entities** (see step 9), so re-saving an already-corrupt file self-heals.
