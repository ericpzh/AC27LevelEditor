...
│ ├── updater.js # Auto-update: HEAD check (R2 ETag), MD5 comparison (prefers the Worker's X-AC27-MD5 on the download GET, falls back to the HEAD etag), exe download ({ filePath, download } + timings), batch script generator
...
│ └── udp_listener.js # UDP telemetry — 10 Hz binary aircraft state on 127.0.0.1:20266 + commands port 20267
├── mcp/
...
│ │ ├── flightDefaults.js # Pure helpers for new flight creation (random airline, cascaded aircraft/reg, non-conflicting stand, airline-aware Language via languageForAirline — CN→zh else en, falling back to Z*→zh, random Voice from dropdown, runway-constrained STAR for arrivals, Airway always cleared for departures)
...
- **Fuzz save test (`tests/e2e/fuzz-save.spec.mjs`, `npm run test:fuzz`):** randomized 50–200 op storms via MCP per production level (sourced from `E2E_GAME_ROOT`), gated before save through the app's own `runTripleValidation` with an auto-repair loop (reg/type/num/STAR/stand/time/**airline-language** fixes; synthetic format-preserving registrations when canonical pools are exhausted), then a real UI save-with-backup + reload verification and a game-compat gate (`runChecks` — duplicate plan keys, docked entities, stand conflicts, `voice-language-mismatch`, **`airline-language-mismatch`**). Generation picks the airline-required Language (CN → zh, every other carrier → en) and the AirlineCode modify op cascades it, so the saved schedules stay game-loadable. `tests/e2e/fuzz-cli.mjs` is the runner wrapper that adds the `--replace` flag (copies PASSED levels' `.acl` + `.acl.bak` + `flight_schedule_*.csv` into the real install; `FUZZ_REPLACE=1` env equivalent). Gated on `FUZZ_RUN=1` — skips in normal `test:e2e` runs.
