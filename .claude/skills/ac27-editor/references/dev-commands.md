...
The release workflow (`.github/workflows/release.yml`) triggers on `v*` tags **or `workflow_dispatch`** (manual with optional `version` input — resolves `tag_name` from the tag or the input). It builds **Windows** (portable `.exe`, 3 variants: `AC27Editor.exe` normal + `AC27EditorVoice.exe` voice via `--voice`, both R2 auto-update, and `AC27EditorWorkshop.exe` workshop via `--workshop` with `resources/workshop.json` marker that disables auto-update — Steam Workshop handles updates, marker travels with the exe so moves are safe), **macOS** (**universal** `.dmg`, **unsigned**), **Linux** (`.AppImage` + `.deb` — no auto-update, release-attached only like macOS), and the **AC27Approach plugin DLL** (`mods/AC27Approach`, net6.0) in parallel (artifacts: `AC27-Editor-Windows` normal+voice, `AC27-Editor-Workshop` workshop-only, plus `release-*` dirs, `tag_name` pinned via `steps.version.outputs.version`). The normal+voice Windows builds are uploaded to Cloudflare R2 (`ac27editor/AC27Editor.exe` + `.md5` and `AC27EditorVoice.exe` + `.md5` in the same bucket) for auto-update delivery **before** the workshop build (so a workshop failure can never block it), and the plugin DLL is uploaded to `s3://ac27approach/AC27Approach.dll` (dedicated bucket, endpoint `https://66f99fd03d3228c43e0acb85f7b8298f.r2.cloudflarestorage.com`) — served through the `https://ericpzh.rest/ac27approach*` Worker route, which the Flight Strips window's Load DLL button pulls from (download-first, file-dialog fallback; see `mods/docs/cloudflare-worker-routes.md`). All normal+voice+plugin artifacts are attached to a GitHub Release with auto-generated release notes (workshop exe is **not** in the Release — Workshop only). **Workshop (build + Steam deploy) is best-effort**: the workshop build/artifact steps are `continue-on-error`, and the release job's workshop deploy steps are gated on the workshop artifact having downloaded, so a Workshop failure is reported but never blocks the GitHub Release or the R2/Cloudflare publish. **macOS is also best-effort**: the mac build/upload steps are `continue-on-error` (+ `if-no-files-found: warn`), the release job's mac download is tolerant, and an **Assemble release file list** step includes `release-macos/*.dmg` only when that artifact actually built — everything else stays required (`fail_on_unmatched_files: true`), so a mac-only failure degrades to a Windows + Linux + plugin release. The `release` job auto-deploys **Steam Workshop** (`appid 3328490`, `publishedfileid 3806070599`): workshop content is `AC27EditorWorkshop.exe` (no auto-update, no Voice) copied into `steam-workshop-content/` (`*Voice*` assert; no stale `AC27Editor.exe` rename) and `upload_mod.vdf` generated with absolute `contentfolder` + the app-icon `previewfile`; content is pushed via `steamcmd +workshop_build_item` with a cached-login `STEAM_CONFIG_VDF`; title `AC27Editor` is constant (`workshop/title.txt`) and bilingual descriptions (`workshop/description_en.txt`/`workshop/description_zh.txt`) are pushed per-language via `scripts/update-workshop-i18n.mjs` (`english`/`schinese` → Steam language switcher, requires `STEAM_PUBLISHER_KEY`, gracefully skipped if missing). `electron/updater.js:71` `isWorkshopBuild()` checks `resources/workshop.json` so workshop builds never prompt even when moved.
...
The release flow only ever *updates* the existing item (app `3328490`, publishedfileid `3806070599`). Creating an item where none exists — or migrating to a different app's Workshop — means handing steamcmd a VDF that sets `appid` and **no** `publishedfileid`:
...
- `CSC_IDENTITY_AUTO_DISCOVERY: false` (Windows jobs) and `mac.identity: null` (`build.js`) both disable code signing since we have no signing certificate — the macOS `.app` ships **unsigned**, so macOS quarantines it on download and Gatekeeper reports *"AC27 Editor.app is damaged and can't be opened"*; users clear it with `xattr -cr "/Applications/AC27 Editor.app"` (README "macOS Gatekeeper" section). The mac target is `arch: ['universal']` (arm64 + x64); note the bundled `ffmpeg-static` stays host-arch, so background-video conversion fails on the opposite architecture.
- Manual release: trigger the workflow via `workflow_dispatch` on GitHub Actions with an optional version input.
- macOS builds produce an unsigned universal `.dmg` (users must clear the quarantine attribute — see above); Windows builds produce a portable `.exe` (no installer); Linux builds produce an `.AppImage` + `.deb` (no auto-update — the updater is win32+portable only).
