...
### 10.1 Tool Coverage (mirrors the painter toolbar)
...
| **Select / Box Select** | `get_ground_painter_state` + `delete_ground_objects` | Selection itself is UI-only; MCP reads the graph via `get_ground_painter_state` and deletes via point query (see below). |
| **Delete (single)** | `delete_ground_objects({target:{x,z}, threshold?})` | Deletes the **nearest** object to `target` using the same priority as the UI Select tool: `stand > segment/runway > area` (`SKILL.md: pickForeground`). `threshold` defaults to 0.6 GU. For runways cascades to strips + threshold nodes; records `meta.deletedPks`/`deletedAreaIds`/`deletedStandNames`/`deletedRunwayNames` and does orphan-node GC so the delete persists through `patchSceneryBlob`; pushes paired history. |
| **Move objects** | `move_ground_objects({targets:[{x,z}×≥1] OR selectAll:true, dx, dz, threshold?})` | Translate 1..N objects (mirrors Select body-drag: single/multi/selectAll). Targets resolve like delete. Runway moves carry coupled pavement strips; areas translate polygon points. Pushes paired history. |
...
| **Rename** | `rename_ground_object({kind:'segment'|'runway'|'stand', idx, name?/names?})` | Rename a segment `Name`, a runway's two end names + `physicalName` (validates `^[0-9]{1,2}[A-Z]?$`, distinct, uniqueness, cascade to strips), or a stand `Name`/`Identifier`. Pushes paired history. |
...
- `get_ground_painter_state` → `{ graph, summary, metaSummary, tool, isOpen, hasEdited, historyDepth }`. `summary`: `{nodes, segments, runways, areas, stands}`; `metaSummary`: `{nodeOrigPk, segOrigPk, deletedPks, deletedAreaIds, deletedStandNames, deletedRunwayNames, runwayPavement, runwayOrigInfo}` (for write-path debugging). If `graph` is `null` the painter is not seeded — open the Ground Painter UI once; the MCP now returns a friendly `{success:false, error:'Ground Painter not yet initialized — open the Ground Painter UI once…'}` instead of a raw `-32603`.

### 10.3 Lifecycle & Guarantees

- **Paired history**: every mutating MCP tool (`create_*`, `create_runways`, `create_taxiway_fillet`, `delete_*`) pushes `groundPainterHistory` **and** `groundPainterMetaHistory` via `_pushPainterHistory` before mutating, so `undo_ground_painter` restores a consistent pair (no meta-graph drift).
- **Meta persistence**: creates push `null` into `nodeOrigPk`/`segOrigPk`/`runwayOrigPk`/`areaOrigId`/`standOrigPk` for new objects; deletes push the survivor's PK/id into `deletedPks`/`deletedAreaIds` and run orphan-node GC (adjusting `runwayPavement` indices), so `patchSceneryBlob` drops them at save — including jetway cascade (`_reconcileJetwayFrames`) for deleted stands and checkpoint-frame reconciliation for runways (`_reconcilePhysicalRunwayFrames`).
