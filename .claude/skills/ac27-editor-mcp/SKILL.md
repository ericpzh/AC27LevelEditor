...
| **Rename (ground)** | `rename_ground_object({kind:'segment'|'runway'|'stand', idx, name?/names?})` | Rename a segment `Name`, a runway's two end names + `physicalName` (validates `^[0-9]{1,2}[A-Z]?$`, distinct, uniqueness, cascade to strips), or a stand `Name`/`Identifier`. Pushes paired history. |
