...
### `send_voice_command`
`{transcript: string}`. Parses a spoken-style sentence against the **live** aircraft list (same pipeline as the PTT mic — `parseVoiceTranscript`) and dispatches the patch frames to the game in real time. Requires the game + BepInEx plugin running so UDP telemetry exists. Returns `{success, callsign, renderedLine, sent: [{type, label}], notices, aircraft}`. Selection-only (bare callsign) → `success:true, sent:[]`. Fails with `success:false` when no aircraft matches, or the aircraft is not on the approach channel (controlSeat 5). Each parse also prints a `[VOICE-PARSE]` line to the editor's main-process log (`npm start` terminal). Example transcript: `"CSC6918: climb and maintain 9000, reduce speed to 180 knots"` (language auto-detected, en/zh).
