...
| **Air Fillet** `F` (air) | `create_airway_fillet({procA, procB, radius?})` | Rounds the corner between two **connected** *airway procedures* sharing a node. `procA`/`procB` are procedure indices; radius 0.5..5.0 (default 2.0). Connected-only (no virtual); validates straight-only, angle 5..175°, parallel guard, truncates both legs at `t1`/`t2` and inserts arc procedure. |
