...
| **Taxiway Curve / Fillet** `F` (rounding) | `create_taxiway_fillet({segA, segB, radius?})` | Rounds the corner between two **straight** segments. `segA`/`segB` are segment indices (0..segments.length-1); `radius` 0.5..5.0 GU (default 2.0). Handles both **connected** (shared node) and **virtual** (disconnected) cases — virtual is additive (no deletions), connected truncates legs and inserts arc. Validates straight-only, angle 5..175°, parallel guard, and ghost-ref repair. |
| **Runway** `R` | `create_runways({runways:[{a:{x,z}, b:{x,z}}]})` | Each runway is a runwayLine `a→b` pair. Names are derived from heading (e.g. `01/19`), width 0.50, and a collinear pavement strip (`Flags=4`, name = physical name, `0.6 GU` overhang) is auto-synthesized and coupled via `meta.runwayPavement` — same as `GroundPainter.jsx:commitRunway`. |
| **Area / Polygon** `G` | `create_areas({areas:[{areaType:0|1|2, points:[{x,z}×≥3]}]})` | `areaType` 0=boundary/perimeter, 1=apron, 2=building (default 2 in UI). Bulk-create; each polygon needs ≥3 vertices. Single-polygon `create_area({areaType, points})` remains as a back-compat alias. |
...
| **Delete (single)** | `delete_ground_objects({target:{x,z}, threshold?})` | Deletes the **nearest** object to `target` using the same priority as the UI Select tool: `stand > segment/runway > area` (`SKILL.md: pickForeground`). `threshold` defaults to 0.6 GU. Records `meta.deletedPks`/`deletedAreaIds` and does orphan-node GC so the delete persists through `patchSceneryBlob`; pushes paired history. |
| **Delete All** | `delete_all_ground_objects({confirm:true})` | Destructive: empties the graph **and** records every survivor's PK/areaId in `deletedPks`/`deletedAreaIds` so the save actually drops them; pushes paired history (undoable). Requires explicit `confirm:true`. |
...
- **Flight reconciliation**: editing the graph does **not yet** reconcile dependent **flights** (plan's flight reconciliation is not implemented in v1).
- **Jetway integrity**: stand deletes via `delete_ground_objects`/`delete_all_ground_objects` record PKs, so the save drops `jetway:*` static items and their checkpoint `jetway:*` runtime entities — no more `Jetway: static item 'jetway:NN' does not exist` at level init.
