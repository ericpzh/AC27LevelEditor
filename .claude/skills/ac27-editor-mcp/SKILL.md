...
## 10. Ground/Air Painter (unified)

Edit the level's **static scenery** (taxiways, runways, areas, stands) and **airways** (FIX waypoints + procedures) via the dedicated Ground/Air Painter window. The graph is **id-free** (`{nodes, segments, runways, areas, stands, airwayNodes, procedures}`); a `meta` side-table carries each object's original `$k`/OsmId/`$id` so survivors keep their ids at save. The mode toggle (`ground` ↔ `air`) is `groundPainterMode` in the store.

First turn on any ground/air task: `get_editor_status → get_ground_painter_state`.

### 10.1 Tool Coverage (mirrors the painter toolbar — ground + air)

| Toolbar Tool | MCP Tool | Notes |
|---|---|---|
| **Air/Ground view toggle** (toolbar, right side) | `set_ground_painter_mode({mode:'ground'|'air'})` | Switches the painter between the ground scenery view and the airside view (mirrors the toolbar's `[data-testid="air-ground-toggle"]`). Returns `{previous, mode}`. **The graph is shared across both modes and the edit tools below are mode-agnostic** — the mode only controls which layers/tools the UI shows, so agents may edit ground + air without switching. Returning to `ground` clears the air runway filter (`groundPainterActiveRunways`). |
| **Taxiway Line** `D` (straight) | `create_taxiway_lines({lines:[{a:{x,z}, b:{x,z}, name?, flags?}]})` | Deduplicates nodes by coordinate (1e-6), validates distinct endpoints, pushes paired `groundPainterHistory`/`groundPainterMetaHistory` so `undo` restores correctly. `flags` defaults to 2 (wider). |
| **Taxiway Curve / Fillet** `F` (rounding) | `create_taxiway_fillet({segA, segB, radius?})` | Rounds the corner between two **straight** segments. `segA`/`segB` are segment indices (0..segments.length-1); `radius` 0.5..5.0 GU (default 2.0). Handles both **connected** (shared node) and **virtual** (disconnected) cases — virtual is additive (no deletions), connected truncates legs and inserts arc. Validates straight-only, angle 5..175°, parallel guard, degenerate-leg guard (collapsed leg/arc → rejected), pre-mutation same-anchor check, and ghost-ref repair. |
| **Runway** `R` | `create_runways({runways:[{a:{x,z}, b:{x,z}}]})` | Each runway is a runwayLine `a→b` pair. Names are derived from heading (e.g. `01/19`) with suffix collision handling (`L/R` then `C` against taken physicalName/end names — avoids duplicate `$k: runway:<name>` → "found 0 named runways"), width 0.50, and a collinear pavement strip (`Flags=4`, name = physical name, `0.6 GU` overhang) is auto-synthesized and coupled via `meta.runwayPavement` — same as `GroundPainter.jsx:commitRunway`. Returns `created:[{names, physicalName}]`. |
| **Area / Polygon** `G` | `create_areas({areas:[{areaType:0|1|2, points:[{x,z}×≥3]}]})` | `areaType` 0=boundary/perimeter, 1=apron, 2=building (default 2 in UI). Bulk-create; each polygon needs ≥3 vertices. Validates `polygonIsSimple` (self-intersecting bowtie rejected — game Triangulator guard, same as vertex drag). Single-polygon `create_area({areaType, points})` remains as a back-compat alias. |
| **Stand** `H` (plane) | `create_stands({stands:[{x,z, heading:1..360}]})` | Nose-anchored; creates `nose`+`tail` nodes (`STAND_LENGTH=0.63`) and a `heading` in 1..360 (0/unset → 360/north via `_normalizeHeading`). Mirrors `GroundPainter.jsx:standHeadingPlacement`. |
| **Airway Node** `N` (air) | `create_airway_nodes({nodes:[{x,z, name?}]})` | Air FIX waypoint. Creates a new airway node per entry; optional `name` defaults to `FIX<n>`; validates finite x/z; pushes paired history (meta `airwayNodeOrigPk` + `null`). |
| **Airway Procedure** `C` (air) | `create_airway_procedures({procedures:[{name, routeType:0..3, runwayName, airwayNodeIdxs:[≥2]}]})` | Chains existing airway nodes into a procedure (STAR 0 / APP 1 / SID 2 / MISS 3). Validates runway exists, idx range, no consecutive dup, duplicate `(name,runway,routeType)` rejected. Pushes paired history (meta `airwaySegOrigPk`). |
| **Air Fillet** `F` (air) | `create_airway_fillet({procedure, vertex?, radius?})` | Intra-procedure fillet (AAA): rounds an interior corner of ONE procedure. Pass `{procedure, vertex}` (`vertex` = position of the corner fix in `airwayNodeIdxs`, `1..len-2`) or omit `vertex` to auto-pick the sharpest filletable corner. `radius` is **50..500 GU** (default 120; clamped down to the leg length). Connected-only (no virtual); validates angle 5..175°, parallel guard; replaces the corner fix with `T1 + arc-interior + T2` nameless nodes and reroutes `airwayNodeIdxs`. **Inter-procedure (`procA != procB`) is rejected** — matching the UI, where after picking an edge of AAA the snap/select is locked to AAA. |
| **Select / Box Select** | `get_ground_painter_state` + `delete_ground_objects`/`delete_airway_objects` | Selection itself is UI-only; MCP reads the graph via `get_ground_painter_state` and deletes via point query (see below). |
| **Delete (single ground)** | `delete_ground_objects({target:{x,z}, threshold?})` | Deletes the **nearest** ground object to `target` using the same priority as the UI Select tool: `stand > segment/runway > area` (`SKILL.md: pickForeground`). `threshold` defaults to 0.6 GU. For runways cascades to strips + threshold nodes; records `meta.deletedPks`/`deletedAreaIds`/`deletedStandNames`/`deletedRunwayNames` and does orphan-node GC so the delete persists through `patchSceneryBlob`; pushes paired history. |
| **Delete (single air)** | `delete_airway_objects({target:{x,z}, threshold?})` | Deletes the **nearest** air object to `target` (`airwayNode > procedure` priority, `threshold` 0.6 GU). Node delete cascades to degenerate procedures (<2 nodes); records `meta.deletedAirwayPks` / `airwayNodeOrigPk`/`airwaySegOrigPk` GC so save persists; pushes paired history. |
| **Move objects (ground)** | `move_ground_objects({targets:[{x,z}×≥1] OR selectAll:true, dx, dz, threshold?})` | Translate 1..N ground objects (mirrors Select body-drag: single/multi/selectAll). Targets resolve like delete (ground). Runway moves carry coupled pavement strips; areas translate polygon points. Pushes paired history. |
| **Move objects (air)** | `move_airway_objects({targets:[{x,z}×≥1] OR selectAll:true, dx, dz, threshold?})` | Translate 1..N air objects (airway node / procedure body). Targets resolve like delete_airway; a procedure move carries its member nodes; `selectAll` moves every airway node. Pushes paired history. |
| **Move endpoint/vertex** | `move_ground_endpoint({target:{x,z}, to:{x,z}, kind:'node'|'areaVertex'})` | Drag a single graph node (segment endpoint, runway threshold, stand nose/tail) or an area vertex to `to`. Runway threshold moves carry strips; validates simple polygon after area vertex moves. Pushes paired history. |
| **Rename (ground)** | `rename_ground_object({kind:'segment'|'runway'|'stand', idx, name?/names?})` | Rename a segment `Name`, a runway's two end names + `physicalName` (validates `^[0-9]{1,2}[A-Z]?$`, distinct, uniqueness; cascades to the coupled pavement strips, the directional `entries`/`exits`, **and `graph.procedures[].runwayName`** — the writer groups each runway's `Routes` by `runwayName`, so a rename that skipped procedures orphaned every STAR/APP route on it and the game NRE'd spawning arrivals that referenced them), or a stand `Name`/`Identifier`. Pushes paired history. |
| **Rename (air)** | `rename_airway_object({kind:'airwayNode'|'procedure', idx, name})` | Rename an airway FIX or its procedure; procedure rename validates duplicate `(name,runway,routeType)`. Pushes paired history. |
| **Undo** | `undo_ground_painter({})` | Depth-1 undo — restores **both** `groundPainterGraph` and `groundPainterMeta` (paired `groundPainterHistory`/`groundPainterMetaHistory`), matching the UI's `Ctrl+Z` (`appStore.js:setGroundPainterGraphWithMeta`). Returns `{undone:true/false}`. |

### 10.2 State

- `get_ground_painter_state` → `{ graph, summary, metaSummary, tool, mode, isOpen, hasEdited, historyDepth }`. `summary`: `{nodes, segments, runways, areas, stands, airwayNodes, procedures}`; `metaSummary`: `{nodeOrigPk, segOrigPk, deletedPks, deletedAreaIds, deletedStandNames, deletedRunwayNames, runwayPavement, runwayOrigInfo, airwayNodeOrigPk, airwaySegOrigPk, deletedAirwayPks}` (for write-path debugging). `mode`: `ground`|`air`. If `graph` is `null` the painter is not seeded — open the Ground Painter UI once; the MCP now returns a friendly `{success:false, error:'Ground Painter not yet initialized — open the Ground Painter UI once…'}` instead of a raw `-32603`.

### 10.3 Lifecycle & Guarantees

- **Paired history**: every mutating MCP tool (`create_*`, `create_runways`, `create_taxiway_fillet`, `create_airway_*`, `delete_*`, `move_*`, `rename_*`) pushes `groundPainterHistory` **and** `groundPainterMetaHistory` via `_pushPainterHistory` before mutating, so `undo_ground_painter` restores a consistent pair (no meta-graph drift).
- **Meta persistence**: creates push `null` into `nodeOrigPk`/`segOrigPk`/`runwayOrigPk`/`areaOrigId`/`standOrigPk`/`airwayNodeOrigPk`/`airwaySegOrigPk` for new objects; deletes push the survivor's PK/id into `deletedPks`/`deletedAreaIds`/`deletedAirwayPks` and run orphan-node GC (adjusting `runwayPavement` indices), so `patchSceneryBlob` drops them at save — including jetway cascade (`_reconcileJetwayFrames`) for deleted stands and checkpoint-frame reconciliation for runways (`_reconcilePhysicalRunwayFrames`) + airway dangling gate for `airway-node`.
- **Node dedup**: `create_taxiway_lines` reuses an existing node when a coordinate matches within 1e-6 (`_findNodeIndexByCoord`), preserving shared junctions; `create_stands`/`create_runways`/`create_airway_nodes` create fresh nodes for their geometry (airway nodes are always new, never deduped).
- **Validation**: distinct-endpoint checks (`Segment needs distinct endpoints`), area ≥3 vertices, `areaType` 0|1|2, runway distinct endpoints, fillet straight-only + angle/parallel/radius guards, airway idx range + duplicate `(name,runway,routeType)`, segment/procedure-index range checks, and `target` type checks all return `{success:false, error}` with `isError:true` (no raw throw).
- **In-memory until Save**: all edits are in-memory until the painter's **Save** (confirm prompt with `.bak` checkbox) — or **Cancel** which discards. The painter renders as a **full-screen fixed overlay** (`z-index:9999`) that visually covers the editor; the MCP API is **not** gated — `create_flights`/`modify_flights` still run while the painter is open. A **0-runway graph cannot be saved**: the painter `save()` blocks with `ground_painter_validation_runway_required`, and the `save-ground-painter-data` handler refuses the write (0 `runway:` static keys) because the game throws `ArgumentException: runway.initialRunways is empty`. A runway created via `create_runways` is auto-added to the level's `InitialRunways` (its primary end), so a fresh runway is active on load.
...
- **Degenerate-edge guards**: node drags freeze when consecutive vertices would colocate; `attachVirtualFilletLeg`/`applyVirtualFillet` reject collapsed legs/arcs and same-anchor fillets before mutating; `create_areas` and area vertex drags reject bowtie polygons via `polygonIsSimple` (`src/acl/scenery_graph.js` + ESM mirror `polygon_simple.js`); air fillet rejects non-connected (virtual) pairs.
...
- **Jetway & airway integrity**: stand deletes via `delete_ground_objects` record PKs, so the save drops `jetway:*` static items and their checkpoint `jetway:*` runtime entities — no more `Jetway: static item 'jetway:NN' does not exist` at level init. Airway node deletes cascade to degenerate procedures (`<2` nodes) and are guarded by the airway dangling gate (`airway-node` → `airway-segment`/`runway Routes`), so no stray `$iref` to a deleted FIX survives.
