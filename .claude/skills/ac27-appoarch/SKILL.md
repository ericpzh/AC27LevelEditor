...
- **`clear_for_appr` semantics (v9, 2026-08-04):** only STAR aircraft (`EAircraftState.Fly`) can be handed off; a heading override is replaced at command time by the **bounded de-snap** (armed at 6b ONLY when the transition confirmed `ApproachState` — a still-STAR aircraft is never fought): each tick the nose rotates at the frame's `rate=N` (default 3°/s, GAME time via `GameDt()`) toward path[0] — the IAF — which IS the game's own pre-capture steering aim (zero fight; without it the game's easing sweeps the nose at ~35°/s, live CJX2697); the AVC target speed is re-lifted to the commanded knots every tick while pre-capture (`Dynamics.AVCController.SetTargetSpeed` — the OPERATIVE speed target; the approach state leaves the stale STAR pace ~2.4 kt in place until capture, so the aircraft otherwise crawls at ~1.24 u/s); and `RescheduleEta` re-asserts the flight-model ETA shift per-tick (the transition-finalize rebuilds the model once from the level schedule and clobbers a dispatch-only shift — live CCA4851: `afmAppT` 00:00:11.7 → back at 00:07:55.867 by watch step 0). **Release ONLY on capture (dist to path[0] < 120 u — `CfaJoinDist`) or the 36k-tick cap (`CfaDeSnapCap`, ~10 min at ×1 — must cover the crawl-speed closure)**; v9 REMOVED the on-aim release (live CCA4851: handed over 700 u out, the game's line-capture swung the nose 21° PAST the IAF bearing — the user's "overshoot the turning direction" — so the de-snap now holds the IAF bearing for the whole pre-capture cruise); a state guard drops the override immediately when the aircraft leaves `Approach` state (`cfa: <CS> de-snap dropped (state → …)` — necessary now the cap is 60× longer). Release log: `cfa: <CS> de-snap released (captured|cap)`; the game's steering flies the final turn at the IAF from there. **Game-time clock (v7):** every rate-driven rotation steps by `GameDt()` = `ContextCross.Clock.GameTime.TimeScale × GameTime.FixedDeltaTime` (resolved lazily from any aircraft's afm `_gameTime`, VContainer `LifetimeScope` scan fallback; logs `game-time: clock resolved — Delta … (stubbed — unused), TimeScale …, IsPaused …` once, or `game-time: clock NOT resolved — fallback dt=…` when the scans find nothing) — `Time.timeScale` reads 1 at any game speed (CES5578 ×10 slow-turn log) and `GameTime.Delta` is a stubbed getter reading 0.00000 live (v6 froze every rotation). **Diagnostics (v8-d, no behavior change):** every 10 de-snap ticks a diag line `cfa: <CS> de-snap diag: hdg …° aim(IAF) …° game …° Δ …° dist …` (Δ = angle between our aim and the game's own steering output, stashed per tick by the channel-lock prefix — the sweep evidence: Δ ≈ 0 through the hold = the zero-fight claim); after a release a 300-step (5 s) post-release observer logs the game-owned nose every 10 steps (`cfa: <CS> post-release: hdg …° pos (…) rbVel …`) — the overshoot window. **Known outstanding (v9 session, out of scope):** the speed lift did NOT take — `rbVel 1.24 u/s` at ×1 with `afmETA 14.1 s` (the crawl persists; next session must confirm the per-tick lift lands). The approach speed is ALWAYS written (240 or the frame's `kts`; log `cfa: <CS> speed: ts=… tts=… dtts=… fwd=True accel 1/-2`) — the kts-only behavior left the channel speed unset and the path-following crawled ~1-4 u/s instead of ~123 (live 2026-08-03); **radio handoff (v2, 2026-08-04):** BOTH `_radioChannel` and `_jurisdictionRadioChannel` are written to the airport's tower channel (`RadioChannelManager.GetResolvedChannel(EChannel.Tower)` via VContainer, RadioSystem `_radioChannelBindings` fallback — logs `cfa: <CS> radio: radio … → TWR(…)` and `cfa: <CS> radio: jurisdiction … → TWR(…)`; resolution failure = logged skip, self-heals on touchdown). v1 wrote only the jurisdiction slot and the aircraft stayed on approach — v2 flips both. Tracer fields: `chTs=`/`chTts=`/`chDTts=`/`chFwd=` (channel speed — the crawl diagnostic), `rbVel=`/`dynVel=`/`dynTs=`/`dynAir=` (ACTUAL motion — rigidbody velocity + the `Dynamics`-level speed fields, the v4 crawl proof: channel 240 + rbVel 1.24 = stuck in pre-capture steering), `afmAdj=`/`afmETA=` (flight-model `SpeedAdjustment`/`ETA` — the reschedule's readback), `rc=`/`jrc=` (radio slots — the re-assert detector).
...
3. The AFTER dump shows state + `route=` + `wait=[…]`; the per-step watch shows whether the transition stuck at the state-object level (`st=FlyApproachState` with `dyn=Approaching` = half-transition). `params-replant:` lines catch the game re-planting the STAR's params — when you see it, the ACTIVE state owns the channel's params (snapshots each step), not the reverse. Since v5 the state-check verdict line is `… — path ok (aircraft's own procedure) / rt ok` (verify-only — no rewriting; a `PATH MISMATCH (Init derived something else than the aircraft's own procedure?)` line is a finding to log, not to force-fix), and the de-snap branch emits `cfa: <CS> de-snap armed: rate N°/s toward IAF …, pre-capture speed … kt`. **Expected healthy sequence (v7, from the README's watch signature):** `flight-model reschedule v2 (per-tick): ETA 475 s → 11.7 s …` at watch step 0 (the dispatch shift was clobbered by the transition-finalize rebuild — the per-tick re-assert converges) → `stPr` = 0.000 during the ~4–5 s pre-capture cruise to the IAF (EXPECTED, not a stall) → `afmETA` ≈ 12 s (was 475) → `rbVel` climbing toward ≈ 123 u/s within the first watch steps → nose rotating ≈ 0.05°/step at ×1 (3°/s de-snap) → `de-snap released (captured)` around dist ≈ 120 u (~5 s in) → the natural final turn at 240 kt, with `post-release: hdg ≈ <IAF bearing>°` lines (no 21° swing = v9's capture-only release holds; the `game-time: clock resolved` line must be present — no clock line = the fallback ran).
