...
- **The active state machine state owns its own captured copies:** `ApproachState._runtimeData` / `_pathPointList` (and `FlyApproachState._flyApproachPathPointList` / `_appPointList`) are `Init`-copied at state activation (level load) and the game's path-following reads THOSE, not the aircraft's data channel. The patch rewrites them on mismatch (`cfa: … state check: … MISMATCH — rewriting`) — the tracer exposes them as `st=… stPath=… stPr=…` plus `dataSame=1|0`. **Join-leg v2 (2026-08-04, live log CSN2197):** the game's own `ApproachState.Init` runs synchronously during the transition fires and re-derives its path from `AppPointList`, mutating the planted params' `PathPointList` IN PLACE back to the plain 5-pt APP list (pointer unchanged → the replant diff stays silent) — that stale path[0] (~890 u away) re-stalled the capture gate ~10 s and made the aircraft turn ~180° back to the stale join point. Three-part fix: (1) step 4d plants the join leg into `AppPointList` (channel + fly-state copies) BEFORE the fires so Init derives join-leg geometry natively — deliberately NOT restored, so any later re-derivation stays consistent; (2) the 6b state check builds a FRESH `ApproachDynamicsParams` (never reuse the in-place-mutated object), re-plants channel + `_runtimeData`, and rewrites the Init-derived fields `_initialPosition` / `_approachDirection` / `_touchDownPosition` / `startingProgress`; (3) per-tick join-leg tracking (`TrackJoinLeg` in `OnAircraftStep`) keeps path[0] + `_initialPosition` + runtime/channel path[0] under the aircraft while the approach holds (watch armed or `stPr < 0.01`) — a command-time path[0] goes stale as the aircraft drifts during the deferred activation, and a stale path[0] is exactly what the game steers BACK to. The canonical list is one native object aliased by all surfaces (`PathPointList`/`FlyApproachPathPointList`/`AppPointList`/state copies — one write to [0] updates every surface still referencing it; the state's Init-COPY and the game's clobbered 5-pt channel list are written explicitly). **`PathMismatch` (the 6b mismatch test) compares count + LAST point only** — [0] is deliberately excluded because the tracker keeps it moving; a first-point-only difference is expected and self-healing. Tracking is gated on the watch (`inWatch`) while the state is still `FlyApproachState` — a reverted/never-transitioned aircraft must NOT have its own STAR steering list corrupted by path[0] writes.
...
3. The AFTER dump shows state + `route=` + `wait=[…]`; the per-step watch shows whether the transition stuck at the state-object level (`st=FlyApproachState` with `dyn=Approaching` = half-transition). `params-replant:` lines catch the game re-planting the STAR's params — when you see it, the ACTIVE state owns the channel's params (snapshots each step), not the reverse. Since 2026-08-04 the state-check verdict line is `… — PATH MISMATCH / rt ok / ch ok` (each surface tested separately via `PathMismatch`, last-point-only), and the cfa watch lines carry the hold diagnostics `stInit=`/`stSP=` (`startingProgress`)/`stA0=<2D>/<3D>`/`rtA0=<2D>/<3D>` (aircraft→path[0] distance on the state's list vs the runtime-data list — the gate-target discriminator: `stA0≈0/15` with `rtA0` large = the gate reads the stale runtime-data path)/`afmRem=`/`afmAppT=` (diagnostic only — `AircraftFlightMetrics` is Init-built from the flight plan, not rebuildable). Expected healthy sequence: `AppPointList overwritten (join leg, N pts)` + `FlyApproachState._appPointList overwritten` BEFORE the fires → state check with `stA0=0.0/15.2 rtA0=0.0/15.2` and no `PATH MISMATCH` → `stPr` advancing in the first watch steps → `cfa-turn: game's approach turn running — tangent swept …°` → `cfa-turn converged` — no 10 s hold, no 180° turn-back. A `params-replant: … ← fresh ApproachDynamicsParams` line in the state check means the game's Init had clobbered the channel (the fresh re-plant fired).
