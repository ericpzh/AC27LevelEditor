...
> channels. **Game-time scaling (dt source v7, 2026-08-04):** the step uses
...
| 9 | **Levels without aircraft.** `Aircraft.Step` isn't reached, dict is empty — no-op. Overrides don't survive level switches (dictionary keyed by instance). | v1.0.1: explicitly cleared at level load (`level reset: …` — `ResetForLevelLoad` on the command service's Start/Dispose + the `RestoreRuntimeData` burst backstop), which also clears the frame dedup so re-sent commands dispatch. Re-apply after load; the overlay re-lists callsigns automatically. |
