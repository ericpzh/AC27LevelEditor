...
| 11 | **No teleport, no dead cruise, no turn-back.** The planted path starts AT the aircraft (join leg, PR=0) — the aircraft never jumps, the approach activates in place, and the steering flies it onto the procedure. (Pre-2026-08-04: the path started at the IAF 300–700 units away and the approach state held PR=0 until capture — minutes of silent cruise. 2026-08-04 v1: the game's `ApproachState.Init` mutated the planted params' path in place back to the 5-pt APP list — the capture gate re-stalled ~10 s and the aircraft turned ~180° back to the stale join point before flying the IAF.) | Resolved by the join leg (step 4b) + v2: join leg planted into `AppPointList` pre-fires (step 4d), fresh-params re-plant + Init-field rewrite in the 6b state check, and per-tick join-leg tracking during the hold. |
...
18. **Join-leg v2 (2026-08-04):** `clear_for_appr|CS` on a state-30 arrival → the log shows `cfa: <CS> AppPointList overwritten (join leg, 6 pts)` + `FlyApproachState._appPointList overwritten (join leg, 6 pts)` BEFORE the transition fires; the state check shows `stPath=len=6 f=(<aircraft pos>)` with **no `PATH MISMATCH`** (Init derived the join leg natively) — or a one-shot `PATH MISMATCH` + rewrite — and `stA0=0.0/15.2 rtA0=0.0/15.2` (path[0] under the aircraft on every surface; `stInit=`/`stSP=` show the Init fields). Watch lines: `stPr` advances within the first steps (gate opened — was frozen at 0.000), `stA0` stays ~0, position deltas keep moving forward — **no heading reversal / no 180° turn-back**. `cfa-turn: game's approach turn running — tangent swept …°` appears at gate-open and `cfa-turn converged` follows — no 60 s phase-2 bound. A `params-replant: … ← fresh ApproachDynamicsParams` line means the game's Init had clobbered the channel (the fresh re-plant fired). A re-command shows `previous join leg stripped` in the 4a log.
