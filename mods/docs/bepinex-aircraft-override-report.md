...
### 4.3 `OverrideController.cs` — the `patchHeading` you asked for (heading-only)

> Decoupled: speed and position are **no longer overridden** — the
...
> Smoothed: with an optional rate the nose no longer snaps to the
...
> channels. **Game-time scaling (dt source v7):** the step uses
> `dt` = **`GameTime.FixedDeltaTime` (STATIC constant 1/60) × `GameTime.TimeScale`**
> — the game's OWN clock service (`ContextCross.Clock.GameTime`, resolved
> lazily from any aircraft's `AircraftFlightMetrics._gameTime`, VContainer
> `TryResolve` fallback), the true per-tick advanced game time (1/60 at ×1,
> 1/6 at ×10, 0 while paused — `IsPaused` gate — → the rotation freezes with
> the game). **Not** `Time.timeScale`: live ×10 testing (CES5578 — the
> 116°-reversal slow-turn report, 0.048°/tick = the unscaled 3°/s) proved
> `Time.timeScale` reads 1 at any game speed; the multiplier lives in
> `GameTime` (the UDP telemetry header mirrors it — `timeScale` offset 33,
> `simFlags` bit 0 = isPaused; the in-process clock is authoritative).
> **v7 correction (live CCA4851/CXA5172):** `GameTime.Delta` is a computed
> property with a stubbed getter — it read 0.00000 s/tick with the sim
> running and froze EVERY rotation in the v6 build (`propHdg` held the start
> heading on both update_heading tests; the cfa de-snap held 277° for all
> 190 watch steps, fighting the game's steering until the 600-tick cap). The
> clock-resolve log line prints Delta/TimeScale/FixedDeltaTime/IsPaused — at
> ×10 it must read `TimeScale 10.0`; pause → `IsPaused True`.
> `CommandedDirection` returns `Current` — the two
> direction-lock prefixes must feed the smoothed value, or the game's
> path-tangent write inside Step would snap the nose back to the full
> command every tick.
        ...
        if (parts.Length >= 2)
        {
            switch (parts[0])
            {
                ...
                case "update_position": // legacy alias — pre-decouple name, kts ignored
                    ...
                    {
                        // Smoothed: only update_heading reads the
...
ipcMain.handle('send-patch-command', async (_e, patch) => {
  ...
  // field; omitted = instant snap). rate (clear_for_appr) = the
...
- **v5/v6 (natural flow — the overhaul):** NO path is planted. The game's own `ApproachState.Init` derives the approach path from the aircraft's OWN `AppPointList` (its procedure from the IAF; `GetRoute` `AirwayNodes` is the logged fallback), and the game's pre-capture steering naturally heads the aircraft toward the IAF when the transition lands — no command-time anchor exists to return to. The join-leg machinery below (prepend, per-tick `path[0]` tracking, release rule, 6b path rewrite, cfa smooth-turn) is REMOVED; the 6b state check is verify-only (`path ok (aircraft's own procedure)`), the `FlyApproachState` gate-bypass carries own-procedure params. **The flight-model reschedule (v1, step 5b):** `AircraftFlightMetrics` paces the aircraft by the plan's arrival time — speed target ≈ `RemainingDistance / ETA`. A plan whose schedule says minutes to touchdown while the aircraft is seconds away (live: CCA4851 `afmETA` 475.87 s for ~1740 u → a ~7 kt pace) paces the aircraft at ~1–4 u/s regardless of path geometry — INCLUDING before the handoff (the `[BEFORE]` dump already showed the STAR-phase crawl). **v6 — the live CJX2697 log exposed two more ceilings and the fast turn:** (1) **the ETA floor** — `AircraftFlightMetrics.ETA` = max(planDelta, `_appRouteTime` − elapsedSinceRouteEntry); the cached `_appRouteTime` (the schedule-derived approach duration — live `afmAppT` 00:08:16.347, frozen all watch) floored the ETA at ~496 s even after v1's plan-anchor shift (ETA moved 1017 s → 496 s and stopped), so the pace stayed ~3 u/s. The reschedule is now **post-Init (6b2, `cfa: <CS> flight-model reschedule v2: …`)** and shifts BOTH the plan anchor (`SetEndTime(Arrival)`, raw-ticks shift — the interop exposes the IL2CPP `DateTime`/`TimeSpan`, no managed operators; `_appRouteTime` written as the interop `Il2CppSystem.TimeSpan` field) by −(ETA − remaining/speed) → ETA = remaining/speed → pace target = commanded speed; an `ETA > newEta + 5 s` guard makes it ~idempotent. **v7 (live CCA4851 — the rebuild clobber):** the dispatch-time shift alone is NOT enough — the transition finalize rebuilds the flight model ONCE from the level schedule (afm `_shouldUpdateMetaData` → `Update()` re-derives plan + `_appRouteTime`; live: the AFTER dump read `afmAppT=00:00:11.7`, watch step 0 back at `00:07:55.867` with `afmRem` 1446.6 → 1614.2), so the reschedule is now the shared `RescheduleEta(ac, kts, tag)` helper called at 6b2 (`dispatch`) AND per-tick from the de-snap branch (`per-tick`) — the idempotent guard fires once after the rebuild and stays silent from there (per-entry log cap: first 3 fires, then every 30th). (2) **the stale AVC speed target** — the OPERATIVE speed is `Dynamics.AVCController.SetTargetSpeed` (absolute knots; the channel fields are display); the approach state leaves the pre-handoff STAR pace (~2.4 kt) in place until capture, so the aircraft moved at exactly the STAR pace (1.24 u/s = rem 1256 / ETA 1017) with every channel at 240 — the de-snap re-asserts the commanded knots every tick while pre-capture. (3) **the bounded de-snap (`rate=N` honored again)** — the game's own pre-capture easing otherwise sweeps the nose at ~35°/s (0.59°/step — the fast-turn report); armed at 6b ONLY when the transition confirmed `ApproachState` (a still-STAR aircraft is never fought), the nose rotates at the frame's rate (3°/s default, GAME time — `GameDt` = `GameTime.FixedDeltaTime × GameTime.TimeScale`, v7; see §5.4's timeScale note) toward the IAF — which IS the game's own pre-capture steering aim, zero fight — and releases on capture (dist to path[0] < 120 u — the game's tangent steering then flies the final turn at the IAF), on-aim (≤ 2°), or the 10 s cap (`cfa: <CS> de-snap released (captured|on-aim|cap)`). Watch signature (v7): the log shows `flight-model reschedule v2 (per-tick): ETA 475 s → 11.7 s …` at step 0 (the dispatch shift was clobbered; the per-tick re-assert converges), `stPr` = 0.000 during the ~4-5 s pre-capture cruise to the IAF (EXPECTED), `afmETA` ≈ 12 s (was 475), `rbVel` → ≈ 123 u/s within the first watch steps, nose ≈ 0.05°/step at ×1 (3°/s), `de-snap released (captured)` ~5 s in (~120 u), natural final turn. (Pre-v5) The planted path started **at the aircraft** (its position prepended as the join leg, PR=0) and ran through its OWN assigned procedure — `FlyApproachDynamicsParams.AppPointList` (the same list the game's transition builds its `ApproachState` from; `GetRoute` `AirwayNodes` is the logged fallback), not the nearest-first-fix GetRoute variant (gotcha #12). The join leg is what made the approach RUN: the approach state holds PR=0 and does not steer until the aircraft captures `PathPointList[0]` — the pre-join-leg plant (live log: CES5578/CSN2197) put that point 300–700 units from mid-STAR aircraft, so the approach sat inert for minutes of dead cruise (CSN2197 hit the 60 s phase-2 bound with the tangent never having swept >2°). Glideslope Y computed by the game from remaining path distance, exactly as it does for ACL state-5 aircraft. **Join-leg v2 (live log CSN2197):** the game's own `ApproachState.Init` runs synchronously during the transition fires and re-derives its approach path from `AppPointList`, mutating the planted params' `PathPointList` IN PLACE back to the plain 5-pt APP list (path[0] ~890 units away; the pointer is unchanged so the replant diff stays silent) — re-stalling the capture gate and producing the 180° turn-back to the stale join point once steering finally engaged. Three-part fix: (1) step 4d plants the join leg into `AppPointList` (channel + fly-state copies) BEFORE the fires so Init derives join-leg geometry natively; (2) the 6b state check builds a FRESH `ApproachDynamicsParams` (never reuses the in-place-mutated object), re-plants channel + `_runtimeData`, and rewrites the Init-derived fields (`_initialPosition`/`_approachDirection`/`_touchDownPosition`/`startingProgress`); (3) join-leg tracking (`TrackJoinLeg`, v4 release rule) keeps path[0] + `_initialPosition` + runtime/channel path[0] under the aircraft while it is still on the STAR in the deferred activation window — the aircraft drifts off a command-time path[0], and a stale path[0] is exactly what the game steers BACK to. When the approach transition LANDS (the first `ApproachState` tick) the join leg is pinned to the aircraft's position ONE final time and tracking STOPS: keeping the per-tick pin any longer is a self-referential trap — the gate holds PR=0 until the aircraft captures path[0], and a path[0] that chases the aircraft can never be "passed", so the capture never completes and the aircraft sits in the game's pre-capture steering at ~1.2 u/s (live log CCA4851: `rbVel 1.24` with `chTs=240` everywhere, `stPr` frozen at 0.004 for all 190 watch steps, tangent never sweeping → 60 s phase-2 bound → the game's ~27-42°/s easing runs the "too fast" turn — the same symptom as the v2 report, now unmasked by v3's lock). Released, the aircraft sits ON a static path[0], the gate opens, and pr advances at the game's own rate.
- **The approach speed is ALWAYS written.** `TaxiSpeed`/`TargetTaxiSpeed`/`DynamicsTargetTaxiSpeed` = the frame's `kts` or the ACL default 240 (`ApproachSpeedKts`) — log `cfa: <CS> speed: ts=… tts=… dtts=… fwd=True accel 1/-2`. The pre-v3 kts-only behavior left the fields untouched and the approach path-following crawled at ~1–4 u/s instead of ~123 u/s at 240 kt (live log: `stPr` advanced 0.005/s — the aircraft crept along the STAR tail for minutes, the "did not override the path" report). The tracer now shows the channel values (`chTs=`/`chTts=`/`chDTts=`/`chFwd=`) in every dump — the crawl diagnostic; the aircraft-level `spd` read does NOT reflect them. **v4: the channel values alone are NOT proof of motion** — the live log showed every channel at 240 while the rigidbody crept at 1.24 u/s (`rbVel 1.24` ≈ 240 kt ÷ 100, `dynVel 0.00`, `stPr` frozen at 0.004): the aircraft was stuck in the game's pre-capture steering because the tracker's per-tick pin kept the capture gate shut (see the join-leg tracking note). The trace now also shows the ACTUAL motion and the dynamics-level speed fields in every dump: `rbVel=` (Aircraft3D rigidbody velocity magnitude), `dynVel=` (`Dynamics.Velocity`), `dynTs=`/`dynDTts=`/`dynAir=` (the `Dynamics`-level speed fields — the game's own surface, distinct from the channel), plus `afmAdj=`/`afmETA=` (the flight model's `SpeedAdjustment`/`ETA` — the §8 item-17 "next suspect" once `chTs=240` is visible).
...
| # | Gotcha | Handling |
|---|---|---|
| 1 | **Game logic still runs.** State machine, radio, runway/stand coordination keep operating during the override; since the decouple, **only the heading is yours** — position and speed are the game's, so the aircraft keeps flying its own route (pointing at the commanded heading) and can still "land"/"reach stand" normally. | Accepted as intended. For total takeover combine with Design B or set `KinematicMode.Value = Forward` (public, unused by game code). |
...
| 9 | **Levels without aircraft.** `Aircraft.Step` isn't reached, dict is empty — no-op. Overrides don't survive level switches (dictionary keyed by instance). | Explicitly cleared at level load (`level reset: …` — `ResetForLevelLoad` on the command service's Start/Dispose + the `RestoreRuntimeData` burst backstop), which also clears the frame dedup so re-sent commands dispatch. Re-apply after load; the overlay re-lists callsigns automatically. |
...
| 11 | **No teleport, no dead cruise, no turn-back.** v5: NO path is planted — the aircraft's own procedure (IAF → threshold) + the game's pre-capture steering naturally head it to the IAF; no command-time anchor exists to return to; the flight-model reschedule keeps it at the commanded speed instead of the plan's pace. **v6 (live CJX2697):** the reschedule alone hit the ETA floor (`_appRouteTime` — the cached schedule duration) and the aircraft still crawled at the stale pre-handoff STAR pace (1.24 u/s) — fixed by the post-Init reschedule v2 (plan + `_appRouteTime` shifted together, 6b2) + the per-tick `AVCController.SetTargetSpeed` lift + the bounded de-snap (rate=N honored). **v7 (live CCA4851/CXA5172):** the transition-finalize rebuild (`_shouldUpdateMetaData` → one-time re-derive from the schedule) clobbered a dispatch-only reschedule — the per-tick re-assert (`RescheduleEta` from the de-snap branch) converges after it; and `GameTime.Delta` read 0.00000 live (stubbed getter) — every rotation froze in v6, fixed by `FixedDeltaTime × TimeScale` (the ×10 turn now completes in ~3.9 real s). (Pre-v5 record: the planted path started AT the aircraft (join leg, PR=0) — resolved via step 4b + v2 join-leg-into-`AppPointList` (step 4d) + fresh-params re-plant + per-tick join-leg tracking. Previously: the path started at the IAF 300–700 units away and the approach state held PR=0 until capture — minutes of silent cruise.) | Resolved by v5 + v6. |
| 12 | **Approach route variants.** A runway can have several APP procedures (same name, different first fixes); nearest-aircraft selection may pick a different variant than the flight plan's STAR intends. |  the planted path comes from the aircraft's OWN `AppPointList` (the variant its flight plan intends — provably the same list the game's own transition builds its `ApproachState` from); the nearest-fix `GetRoute` pick only names the route label and backs the fallback when AppPointList is unavailable. `apprName` still forces a specific procedure. |
...
13. **Smooth turn (dt v7):** send `update_heading|CS|dx|dy|3` → the diag lines' `propHdg` drifts toward the commanded heading at ~0.05°/tick at ×1 (3°/s ÷ 60 TPS — `GameTime.FixedDeltaTime × TimeScale`-stepped) while `spd`/`view3D-pos`/`dynVel` keep advancing (the aircraft flies its own route during the turn); the map's nose triangle rotates through intermediate headings — no one-frame jump. At ×10 game speed the turn completes in ~3.9 wall-seconds for a 116° reversal (the same GAME time as ×1 — the v7 fix for the CES5578 slow-turn report; the clock-resolve line reads `TimeScale 10.0` at ×10); while paused the rotation freezes with the game and resumes from where it stopped. A mid-turn re-command continues rotating from the intermediate heading (no snap back). Omitted rate → instant, exactly as before. (v6 regression record: `GameTime.Delta` read 0.00000 live — stubbed getter — and `propHdg` held the start heading at every diag step on CCA4851/CXA5172; the log line now prints Delta/TimeScale/FixedDeltaTime/IsPaused once at clock resolve.)
14. **Smooth clear_for_appr (machinery; v6 = the bounded de-snap):** `clear_for_appr|CS|rate=3` (or the composer's Clear for Approach, which always sends it) → the log shows `cfa: <CS> de-snap armed: rate 3°/s toward IAF <pt>, pre-capture speed 240 kt` (only after the state check confirmed `ApproachState`), then the watch lines' heading drifts from the STAR heading toward the IAF bearing at ~0.05°/tick at ×1 (3°/s of GAME time — `GameDt`) while the position closes on path[0] at the commanded speed; expect `cfa: <CS> de-snap released (captured|on-aim|cap)` around dist-to-path[0] ≈ 120 u — the game's steering then flies the final turn at the IAF seamlessly (no one-frame jump, no 35°/s game-easing segment during the cruise). **v7 speed expectations in the watch:** `flight-model reschedule v2 (per-tick): ETA 475 s → 11.7 s …` at step 0 (the dispatch shift was clobbered by the transition-finalize rebuild — the per-tick re-assert converges), `afmETA` ≈ 12 s at step 10 (was ~475 s), `rbVel` climbing to ≈ 123 u/s within the first steps (the stale STAR pace lifted), `stPr` = 0.000 during the ~4-5 s pre-capture cruise (EXPECTED), `de-snap released (captured)` ~5 s in. The v2/v3 sweep-gate log lines below belong to the pre-v5 machinery (design record). Game-time/pause rules apply (×10 compresses the wall-clock turn; pause freezes it). A cfa without a rate field still de-snaps at the plugin default (3°/s). Backward-compat guard: a bare numeric field on a cfa frame is still the kts approach speed — `rate=3` bare would be misread as 3 kt, hence the keyed form.
15. Rollback: with the plugin disabled, both frame types are inert (select miss / one `UnknownCommand` log line) — nothing crashes.
16. **Radio handoff (v2):** `clear_for_appr` on a state-30 arrival → the log shows `cfa: <CS> radio: radio <apprPk> → TWR(<twrPk>)` AND `cfa: <CS> radio: jurisdiction <apprPk> → TWR(<twrPk>)`; the AFTER dump and ALL watch lines keep `rc=Tower/<twrPk> jrc=Tower/<twrPk>` (the re-assert detector — a flip back names the culprit flow via the line before it). The editor map's strip shows BOTH seats flip to TWR within a telemetry tick. Resolution failure path: log `cfa: … radio: tower channel NOT resolved …`, aircraft behaves exactly as before (self-heals on touchdown).
17. **Approach speed (v4):** `clear_for_appr|CS` WITHOUT a kts field → the log shows `cfa: <CS> speed: ts=240 tts=240 dtts=240 fwd=True accel 1/-2`, and the watch lines show the position advancing ~2 u/step (123 u/s ÷ 60 TPS) with `stPr` ~0.01/step and `chTs=240.0 … chFwd=True` in every dump — no crawl. With `kts=200` the speed log and `chTs`/`chTts`/`chDTts` show 200. **The crawl DID persist with `chTs=240.0` visible (live log CCA4851: `rbVel 1.24` ≈ 240 kt ÷ 100, `dynVel 0.00`, `stPr` frozen at 0.004 for 190 ticks)** — the channel values alone are not proof of motion; the root cause was the join-leg tracker's per-tick pin keeping the capture gate shut (item 18's v4 fix). Verify motion with the new trace fields: `rbVel=` ~123, `dynVel=` non-zero, `dynTs=`/`dynDTts=`/`dynAir=` matching the channel, and `stPr` advancing ~0.0028/tick (0.17/s) — the gate opened. If a crawl still appears with ALL of those correct, the flight-plan metrics (`afmAdj=`/`afmETA=` — `SpeedAdjustment`/`ETA` in `ApproachState.afm`) are the next suspect.
18. **Join-leg tracking (v4 release rule):** `clear_for_appr|CS` on a state-30 arrival → the log shows `cfa: <CS> AppPointList overwritten (join leg, 6 pts)` + `FlyApproachState._appPointList overwritten (join leg, 6 pts)` BEFORE the transition fires; the state check shows `stPath=len=6 f=(<aircraft pos>)` with **no `PATH MISMATCH`** (Init derived the join leg natively) — or a one-shot `PATH MISMATCH` + rewrite — and `stA0=0.0/15.2 rtA0=0.0/15.2` (path[0] under the aircraft on every surface; `stInit=`/`stSP=` show the Init fields). During the deferred activation window (still `FlyApproachState` in the dump) `stInit=`/`stA0=` keep tracking the aircraft — the deferred-window pin. **At the transition the pin is released (v4):** the `stInit=`/`stA0=` values FREEZE at the transition-time position (no more per-tick re-anchoring — the v2-era continuous pin kept the capture gate shut: PR=0 forever, `rbVel 1.24`, tangent never sweeping — live log CCA4851). Watch lines: `stPr` advances within the first steps after the release (gate opened — was frozen at 0.000), `stA0` stays ~0, position deltas ~2 u/step — **no heading reversal / no 180° turn-back / no 1.24 crawl**. `cfa-turn: game's approach turn running — tangent swept …°` appears at gate-open and `cfa-turn converged` follows — no 60 s phase-2 bound. A `params-replant: … ← fresh ApproachDynamicsParams` line means the game's Init had clobbered the channel (the fresh re-plant fired). A re-command shows `previous join leg stripped` in the 4a log.

---

## Appendix — source of truth

All class/member references verified against the Cpp2IL dump (`%TEMP%\cpp2il\out\DiffableCs\`):
