 ...
 * The test NEVER modifies the original files — all writes go to temp files in tests/integration/.
 *
 * Usage: node tests/integration/test_e2e_save_load.js --acl <path-to-.acl-file>
 *
...
for (let i = 2; i < process.argv.length; i++) {
  if (process.argv[i] === '--acl' && i + 1 < process.argv.length) {
    ...
    console.log('Usage: node tests/integration/test_e2e_save_load.js --acl <path-to-.acl-file>');
...
if (!aclOriginal) {
  ...
  console.error('Usage: node tests/integration/test_e2e_save_load.js --acl <path-to-.acl-file>');
  process.exit(1);
}
