...
```bash
npm run test:all      # Full suite: Vitest (2216) + save integrity (27) + jetway rebuild (27) + runway pairs (5) + E2E (18, ~8 min)
npm test              # Vitest component + store + utility + electron + integration + MapWindow + updater tests (2350 tests, 130 files, ~40s)
...
node tests/integration/test_api_server.js      # MCP/API tests: 145 tests (~1s)
...
**Last full verification (2026-09-19):** Vitest 2170/2170 (115 files); integration scripts all green
...
**Vitest re-run (2026-09-20):** Vitest **2181/2181** (116 files) green after the first-launch
language picker + Workshop-only auto-detect work (new `LanguagePicker.test.jsx` — 4 tests;
`SetupScreen.test.jsx` — 6 tests) and the dead-code cleanup. Integration/E2E scripts not re-run.

**Auto-update verify hardening (2026-09-21):** `downloadUpdate()` now resolves `{ filePath, download }`
(the GET response's `etag`/`lastModified`/`ac27Md5`/`contentLength`/`received` + `startedAt`/`receivedAt`),
and the `download-update` handler verifies the written file against the Worker's new `X-AC27-MD5`
response header (**preferred** — read from the same GET that streamed the bytes, so a release published
mid-download can't cause a false mismatch) with a fallback to the post-download `HEAD` `etag` for older
Workers; both hashes + both response timestamps are logged to `updater.log` and returned as `diagnostics`
on `UPDATE_MD5_MISMATCH`. The Worker (`mods/docs/cloudflare-worker-routes.md`) now emits `X-AC27-MD5` on
`GET /editor`, drops the `HEAD` cache to 300 s, and keys the download cache on the exe's R2 ETag — fixing
spurious mismatches from out-of-step cached exe/sidecar entries. New `downloadUpdate` tests (3). Vitest
...
**Ground Painter scenery-save hardening (2026-09-23):** four save-blocker classes reported
against a heavily edited `ZSJN_leisure_1.acl` are fixed and covered.
(1) Deleting every taxiway that serves a runway orphaned the runway's `Entries`/`Exits`
`$iref`s, so the survivor gate dropped the whole runway and the save was refused with
"at least one runway". Those elements are now **pruned per element** (runway kept; a runway
with empty Entries/Exits is valid) with a `gate_runway_entries_pruned` warning.
(2) A NEW runway whose threshold indices no longer resolve is dropped with a structured
`new_runway_dropped` warning instead of vanishing (and the renderer re-checks the runway count
after the pre-save ghost repair, plus a same-name end guard), so the IPC refusal became
actionable.
(3) Hardcoded per-scope `$type` fallbacks collided — `33|List<Vector3>` vs
`33|Dictionary<string, IStaticItem>` when re-adding an Area after deleting them all
(`Type id 33 claimed by both ...`), and the PhysicalRunway runtime `3` vs the RuntimeEntities
scope's `3|Jetway`. Every such fallback now resolves against the owning `$blobdoc` scope's
own type table first, else mints a **fresh id above that scope's max** carrying the canonical
name (areas, stands, runway sub-objects, runtime entities). The same fix unblocked adding a
runway entrance/exit to a level whose Entries/Exits had been emptied (the element type is
neither sampleable nor in the table).
(4) Odin registers a type inline only on its FIRST occurrence and emits a bare
`"$type": <id>` afterwards, so areas saved with a fresh id were invisible to both editor
readers (`scenery_graph.js:_isAreaEntity`, `scenery.js:_parseAreas`) while the game rendered
them — bare ids now resolve through the blobdoc table in both readers.
New coverage: `tests/unit/scenery-type-resolution.test.js` (14),
`tests/unit/scenery-area-bare-type.test.js` (7),
`tests/integration/runway_gate_synthetic.test.js` (3),
`tests/integration/runway_entries_prune_on_taxiway_delete.test.js` (2),
`tests/integration/area_readd_after_delete_all.test.js` (2),
`tests/components/EditorScreen/GroundPainter/saveRunwayGuards.test.jsx` (2), plus updated
`runway_entry_type_id` / `scenery_physical_runway_cleanup` assertions.
`v2_frame_reconciliation` now skips cleanly when the editor-managed live level has fewer than
two flights (a scenery delete legitimately purges flights). Full suite green: Vitest **2350 pass / 7 skipped (130 files)**.
...
used `dismiss`). A later cleanup (the Workshop-only SetupScreen rework) dropped `setup_sub`,
`setup_detected_title` and `setup_detected_hint`, leaving **729 zh / 730 en** today. Dynamic key builders (`field_` + column, `airport_` + ICAO, `level_name_` + filename,
...
**Airline/language invariant fix:** the fuzz surfaced
`InvalidOperationException: Flight plan 'B-40N0' failed to allocate callsign 'CAL2017' for crew voice 'CN-Captain-Young'`
at ZGSZ. The game's `CallsignService` allocates a callsign from the pool shared by the captain voice and
the ATC voice, and a callsign clip exists for exactly one captain language, so a flight's `Language` must
follow its callsign's airline — **CN carriers speak zh, every other carrier speaks en** (CAL (TW) / CPA (HK)
are `en` in every game-authored level; a CN carrier only gets `zh` when the airport ships `audio_clips_zh.json`).
Fixed by loading the install's `airline_country_registry.cfg` as `airportValues._airlineCountries`
(`gamePaths.airlineCountryRegistryPath` + `parseAirlineCountryRegistry`/`loadAirlineCountryRegistry` in
`src/acl/utils.js`), a shared rule in `src/utils/airlineLanguage.js` (`languageForAirline` /
`expectedLanguageForFlight`) with a CJS mirror `languageForAirlineCode` for the backend/MCP, new-flight
defaults, the FlightTable Language/Voice dropdowns, `cascadeAirlineLanguage` (Cascade 2b in
`appStore.updateFlight`), `val_airline_language_mismatch`, and the save pipeline's
`_normalizeFlightsForGameCompat` section 4b (runs BEFORE the voice repair so the voice follows the new
language). **MCP:** `buildConstraints` carries `airlineCountries`/`voiceLanguages`/`languages`,
`get_airport_info` + `get_airport_values` expose `airlineLanguages` (per-airport code → required language),
`validateFlightObjects` rejects `airline_language_mismatch`, and `applyCascades` mirrors Cascade 2b.
**Fuzz:** `runChecks` reports `airline-language-mismatch` (fed `airlineCountries` + `languages`),
`fuzz-save.spec.mjs` generates the airline-required Language and repairs the mismatch in its save-gate loop.
Coverage: `tests/unit/airline-language.test.js` (new), `tests/integration/voice_language_normalization.test.js`
(normalizer + `runChecks` + cross-platform registry path resolution for Win/Linux and both macOS layouts),
`tests/utils/validators.test.js`, `tests/store/flightCascade.test.js`, `tests/store/flightDefaults.test.js`,
`tests/store/appStore.test.jsx`, `tests/components/EditorScreen/FlightTable/FlightTable.test.jsx`,
`tests/unit/game-paths.test.js`, and `tests/integration/test_api_server.js` (145 assertions).
Verified with `FUZZ_RUN=1 FUZZ_REPLACE=1` on all 12 browser-visible ZSJN + ZGSZ levels.

---

## Layer 1 - Vitest Component Tests (2460 tests, 134 files)
...
### `npm test` - 2460 pass + 6 skipped (134 test files; includes the Ground Painter scenery suite + airway roundtrip + the full Livery page suite + the aviationstack realtime-import suite + the Workshop publish suite)
...
| `components/SetupScreen/SetupScreen.test.jsx` | 6 | **Game root auto-detection (4):** Workshop build detects a game root and the confirm button starts the cache build (`init-airport-cache` → screen `browser`); **the normal (non-Workshop) build never calls `detect-game-root`** (manual picker only, no detected panel); no detection falls back to the picker; the manual picker result proceeds when detection found nothing. **Workshop layout (2):** no setup subtitle/manual hint, detected panel rendered above the Steam block with parallel Use/picker buttons (`.setup-detected-actions`, 2 buttons); standalone picker kept when nothing is detected. |
| `components/LanguagePicker/LanguagePicker.test.jsx` | 4 | **First-launch language picker (1):** brand + 中文/English buttons, a choice persists via `save-cached-lang`. **I18nProvider status (3):** `unset` when neither localStorage nor cache.json has a language; `chosen` after adopting the cached `en`; `chosen` immediately from localStorage (no `get-cached-lang` call). |
| `components/BrowserScreen/BrowserScreen.test.jsx` | 39 | **Help Button (5):** renders in header, click opens overlay, Escape closes, backdrop click closes, close button works. **Debug Mode Toggle (5):** renders toggle button, shows active state when installed, tooltip on hover, disabled while loading, hidden on non-Windows platforms. **Livery Button (3):** renders button, tooltip on hover, navigates to livery screen. **Tooltips (5):** Change Folder/Language/Help tooltips, hides on mouse leave, switching hover updates text. **Demo File Filtering (4):** hides whitelisted .demo files in non-demo mode, hides non-whitelisted .demo files, shows whitelisted .demo files in demo mode, sorts levels by whitelist order (not start time). **Collapsible Airport Cards (7):** header click collapses/expands a card (radar toggle buttons don't collapse it); auto-collapse collapses trailing airports so all four headers stay visible; auto-collapse is one-shot per app session (a resize after load does not re-run it); auto-collapse result and a manual collapse choice both persist across a browser remount (same session, via the store). Fixtures mock `HTMLElement` `clientHeight`/`offsetHeight` so the fit pass has real geometry in jsdom. **Level-scan loading overlay (1):** while the airport scan is in flight a fixed `.browser-scan-overlay` (blurred backdrop + centered spinner + `browser_loading`) covers the whole screen and the main list renders nothing; it clears once the scan resolves. **Radar/strip mod gate (5):** the toggles render only on Windows (`get-system-info.platform`) and are hidden on mac/linux (as is the Debug Mode settings item); a deployed plugin DLL whose MD5 matches the release reference (bundled Workshop DLL on Steam, R2 object on a normal build) opens the radar with no install; a packaged MD5 mismatch installs from that reference; dev (`!isPackaged`) never enforces the MD5; Debug Mode off shows the "shut down the game and enable debug mode" notice and neither installs nor opens. |
...
| `components/BrowserScreen/BrowserHelpOverlay.test.jsx` | 12 | Help overlay renders title + section headings (Header Buttons/Settings Menu/Levels + the now-enabled Airport cards section), all button descriptions, inline button icons, Escape/backdrop/close-button dismissal, Chinese translations; hides the `modOnly` BepInEx rows (Airport cards section + Debug Mode settings row) on non-Windows platforms (`isWindows={false}`) |
...
| `components/BrowserScreen/BepInExInstallOverlay.test.jsx` | 8 | Progress bar + percentage; success closes overlay; error on failure; Escape closes error; close button works; localized NO_GAME_ROOT error; localized BEPINEX_ALL_SOURCES_FAILED error; progress events update UI |
...
| `components/EditorScreen/FlightTable/FlightTable.test.jsx` | 12 | **Airline-driven dropdowns (3):** the Language cell offers only the airline-required language (non-CN → `en`, CN at a Chinese airport → `zh`) and the Voice cell filters to it, with the legacy current value kept selectable so the exact `CAL2017`/`CN-Captain-Young` row never renders blank. Click on data cell → no selection toggle; checkbox click → toggles; drag from data cell → range-selects; dropdown/time cell clicks → no toggle; clock portal click → no toggle; **aircraft type dropdown lists every `vals.AircraftType` regardless of `_compat.airlineToAircraft`** |
...
| `electron/bepinex.test.js` | 34 | checkStatus (null, partial, full, empty); findBleedingEdgeUrl (URL extraction, artifact not found, HTTP error); findThunderstoreUrl (API parse, non-JSON, missing latest); findDownloadUrl fallback chain (prefers Bleeding Edge, falls back to Thunderstore when the build server fails, BEPINEX_ALL_SOURCES_FAILED when all sources fail); downloadZip (happy path — file content + progress 0→100%, incremental multi-chunk progress, HTTP 404 rejects + file cleanup, network error rejects + cleanup, timeout rejects + cleanup); extractZip (non-Windows guard); installFiles (subdirectory, missing items, flat structure); removeFiles (all items, partial, non-existent); installLatest (full pipeline incl. downloadZip, error cleanup, download progress normalization) |
...
| `unit/create-workshop-item.test.js` | 10 | **Workshop item bootstrap script** (`scripts/create-workshop-item.mjs`) — `buildWorkshopVdf` emits a creation VDF (appid, no `publishedfileid`) or an update VDF (with it), skips empty optional fields, and escapes backslashes/newlines/tabs while downgrading `"` to `'` (steamcmd's KeyValues parser chokes on `\"`) so a multi-line description stays a single parseable VDF line; `parsePublishedFileId` reads the id steamcmd writes back (null when absent); `buildSteamCmdArgs` builds the `+login <user> +workshop_build_item <vdf> +quit` list; `parseArgs` defaults visibility private + run false, accepts multi-word values + `--run`, and rejects a missing value / unknown positional. |
...
| `unit/steam-workshop.test.js` | 36 | **Workshop publish module** (`electron/steam-workshop.js`, node env, injected fake `steamworks` client — legacy `init()→client` and modern namespace shapes) — `isAvailable` (`STEAM_UNAVAILABLE` without the module / init throw, `NO_LICENSE` without game ownership); **single shipping-app constant host (`3328490`, inited exactly once — no cross-app/Spacewar (`480`) fallback probe)**, `NO_LICENSE` vs `STEAM_UNAVAILABLE` told apart by ownership of that same app; **foreign napi codes sanitized (`GenericFailure` → `STEAM_UNAVAILABLE`, detail preserved) + `toPublicError` shape**; `.workshop.json` sidecar round-trip + missing/corrupt tolerance; `parseWorkshopId` (bare id, `?id=` and `/filedetails/` URLs, garbage → null); `readPublishInfo` guards + manifest defaults + sidecar-over-manifest + live-over-sidecar precedence (failed live lookup keeps the sidecar) + **a recorded id whose item was deleted is forgotten (`publishedFileId`/`url` cleared)** + **a recorded id from a previous host app (`appId` != the current host) is ignored — `publishedFileId`/`url` cleared, no cross-app live lookup, other sidecar values still prefill** + **the saved preview image is returned as `previewDataUrl`** + **returns `airline`/`targetPlaneId` (and no composed default title — the renderer localizes it)**; `publishLivery` create-then-update with temp-dir cleanup + sidecar write, update-without-create on repeat (**incl. a legacy sidecar with no `appId`**), **a foreign-app sidecar (`4004140` Playtest → `3328490`) is never existence-checked and republishes as a new item under the current host** **the recorded id is existence-checked first (a deleted item republishes as a new item) and an update that fails because the item vanished creates a replacement**, **Steam's preview-limit error (`GenericFailure: limit exceeded` — preview >1 MiB or Steam Cloud quota full) maps to `PREVIEW_LIMIT`**, **the preview image is saved into the livery folder (`.workshop-preview.<ext>`) and reused on the next upload**, **the final preview is synced into the uploaded content dir (first upload with a newly picked image + repeat reuse, `.workshop.json` never shipped)**, **an oversized preview is downscaled/re-encoded below 1 MiB before submit — the shrunk temp path (not the caller's file) is what reaches `updateItem`, and the shrunk image becomes the remembered `.workshop-preview`**, **auto-association from the recorded sidecar only (`linkItemId` is ignored, never links)**, `BAD_TITLE`/`NO_LICENSE`/`CREATE_FAILED`/`STEAM_AGREEMENT`/`UPLOAD_FAILED` mapping, progress forwarding, caller-supplied preview left on disk. |
| `unit/livery-workshop-packaging.test.js` | 11 | **Workshop content packaging** (`electron/livery.js`, node env) — `workshopModName` sanitization; `buildWorkshopContent` coded errors + **the ENTIRE livery folder copied verbatim (extra files + subdirectories + the saved `.workshop-preview.*`; only the private `.workshop.json` sidecar is excluded) + synthesized mod_info** + **the saved preview never counts as a texture** + multi-part textures + temp cleanup; `buildWorkshopPreview` coded errors + file output (raw-bytes fallback without Electron) + **the injected-`nativeImage` branch resizes to the target width and writes a JPEG (`resize({width, quality:'best'})` + `toJPEG(85)`)** + cleanup; **`ensurePreviewUnderLimit`** — a sub-1MiB file is untouched, an over-limit image is re-encoded below the cap (injected fake nativeImage) and its temp cleaned up, an undecodable image falls back to the original. |
...
| `integration/voice_language_normalization.test.js` | 25 | **Airline/language (10):** the normalizer repairs a non-CN carrier (CAL) from zh→en + en voice, a CN carrier from en→zh at a zh-capable airport, leaves CN→en at an en-only airport, never touches an unknown airline, and no-ops without the registry. `runChecks` reports `airline-language-mismatch` for CAL+zh / CCA+en (zh-capable), accepts CAL/CPA+en and CCA+en (en-only), and skips without the registry. **Cross-platform paths (4):** `_loadAirlineCountryRegistryForLevel`/`_airportSupportsChinese` resolve a Win/Linux tree and both macOS `.app` layouts (BOM+CRLF included). **Voice/language regression** — the game's `VoiceCatalog` throws `InvalidOperationException` when a captain voice's language differs from the flight's `Language`. `_normalizeFlightsForGameCompat` repairs an `-EN` voice on a `zh` flight (and vice versa), a voice unknown to the catalog, never picks an `atc`-role voice, prefers a same-language voice already used in the level, leaves a consistent voice untouched, no-ops without a catalog, and repairs every mismatch in a mixed batch (8). `runChecks` reports `voice-language-mismatch` when the catalog is supplied, stays silent when voice/language agree, and skips the check with no catalog (3). |
...
| `integration/scenery_physical_runway_cleanup.test.js` | 24 | **Runway delete/rename reconciliation (fully synthetic fixture — ships)** — checkpoint-frame `physical-runway:*` drop after a runway delete, rename remap, `addMissing` (synthesize a `PhysicalRunway` runtime entity when a frame has none; the runtime type id is resolved **within the RuntimeEntities scope** — its own registration for the canonical name, else a fresh id above that scope's max — never the old hardcoded `3`, which collided with the scope's `ContextCross.Models.Jetway` and aborted the encoder with `Type id 3 claimed by both ...`; asserted via a per-scope "one id → one name" invariant), `_remapRunwayNameFields` (Runway/RelatedRunway/_departureRunway/_arrivalRunway/InitialRunways cascade), `_remapTaxiwaySegmentName` (oldPhysToNewPhys + end-name fallback), `meta.runwayPavement` population + runway move/reshape/add. The synthetic ACL embeds a `RunwayTimeline`, and the runway-add test asserts the new runway's primary end is auto-appended to `InitialRunways`. |
| `integration/runway_entries_prune_on_taxiway_delete.test.js` | 2 | **Runway survives deletion of all serving taxiways (real fixture)** — deleting every taxiway that serves a runway orphans its `Entries`/`Exits` `$iref`s; the survivor gate used to drop the whole runway ("referenced deleted node(s) … with no repairable replacement"), leaving zero runways and tripping the save-time "at least one runway" refusal. The `Entries`/`Exits` ELEMENTS are now pruned (with a `ground_painter_writer_gate_runway_entries_pruned` warning) and the runway is kept (a runway with empty Entries/Exits is valid). Second case: a mid-field taxiway delete leaves the runway untouched (no prune warning). Game-root gated. |
| `integration/runway_gate_synthetic.test.js` | 3 | **Runway gate behaviour (fully synthetic — ships, no game install)** — the same entries-prune path on a synthetic ACL (`pruned 2 runway entrances/exits`, Entries/Exits emptied, structural thresholds intact, `11` reparse keeps `01/19`); a threshold node still referenced by a surviving runway is **rescued** by `patchSceneryBlob` (runway + node survive, no drop warning); a NEW runway whose threshold indices do not resolve is dropped with a structured `ground_painter_writer_new_runway_dropped` warning instead of vanishing silently. |
| `integration/area_readd_after_delete_all.test.js` | 2 | **Area delete-all → re-add (type-id collision + bare-`$type` read-back)** — after deleting every Area, `_sampleAreaShapes` used hardcoded fallback ids 31/32/33; on ZSJN id 33 is `Dictionary<string, IStaticItem>`, so the re-added Area aborted the encoder with `Type id 33 claimed by both ...`. Fallbacks now resolve against the file's blobdoc table, else a fresh id above the scope max (also covered for `_sampleStandShapes`). The fresh id is registered inline on the FIRST area entry only, so siblings carry a bare `"$type": <id>` — both readers (`scenery_graph.js:_isAreaEntity`, `scenery.js:_parseAreas`) must resolve bare numeric ids through the blobdoc table or the saved areas are invisible to the editor while the game renders them. Asserts patch + binary encode + decode round-trip and both readers. Game-root gated. |
| `unit/scenery-type-resolution.test.js` | 17 | **Scope-local `$type` resolution + runway Entry/Exit prune (pure unit)** — `_scopeMaxTypeId` (highest of inline `"N\|Name"` and bare `N` forms, whitespace before `}` tolerated, 0 on empty), `_resolveTypeOrFresh` (file-table hit wins without consuming a fresh id; undeclared name → fresh id above the scope max; one fresh id per name per patch; legacy fallback only with no scope info), `_sampleAreaShapes`/`_sampleStandShapes`/`_sampleRouteShapes` (inline wins; table resolution; fresh minting, never `$type: 0`, distinct ids per sub-object), and `_pruneRunwayEntryExitElements` (drops only elements touching a dead taxiway node, recomputes each wrapper's `$rlength`, can empty a wrapper, leaves `ThresholdPoints` and unaffected wrappers byte-identical). |
| `unit/scenery-area-bare-type.test.js` | 7 | **Area detection with a bare `$type`** — `_isAreaEntity` accepts the inline name form and legacy 30/31 ids, resolves a bare id through the blobdoc type table, and rejects a bare id mapping to a non-Area type (or with no table). A minimal ACL whose first Area registers a fresh id and whose sibling references it bare is read by BOTH `buildSceneryGraph` and `scenery.js:_parseAreas`; swapping the registration name for a non-Area type makes neither reader claim it. |
| `components/EditorScreen/GroundPainter/saveRunwayGuards.test.jsx` | 2 | **GroundPainter `save()` pre-flight guards (jsdom + real store + Modal)** — a runway whose two ends share a name (27/27) is refused inline (`same name`) without calling `saveGroundPainterData`; a graph whose only runway is dropped by the pre-save ghost repair (threshold PK in `deletedPks`, no live twin) is refused with the "at least one runway" message before IPC, and the in-memory graph ends with zero runways. |
...
| **Livery page (custom liveries):** | **487** | |
...
| `components/LiveryScreen/UploadLiveryDialog.test.jsx` | 12 | **Workshop upload dialog** — loading spinner; title/description/visibility/tags prefilled from `get-workshop-publish-info`; **a first upload's default title is localized from the airline display name + compact aircraft type (`Air China A-320neo Livery` / `中国国航 A-320neo 涂装`), while a previously published title wins**; **the submit button is always labelled Upload (never renames to Publish-new-version)**; **no change-note input**; **a known item's URL is shown up front (clickable)**; submit disabled + reason shown when Steam is unavailable; progress bar driven by `workshop-upload-progress` events + success view with the clickable item URL (**no separate Open-in-Steam button**); error keeps the typed input for retry; **no link field — association is recorded automatically after upload (`linkItemId` is never sent)**; picked preview file travels with the `publish-livery` payload; **unknown error codes fall back to the generic failure text (never a raw key) with the raw code + detail underneath**; **stale-main banner when the debug handshake is missing**. |
...
| `electron/workshop-dll.test.js` | 10 | **Workshop bundled-DLL resolver** (`electron/workshop-dll.js`, node env, electron primed) — `resolveWorkshopBundledDllPath` build gate (null on a non-Workshop build); the game-root path resolves the item DLL via the real `livery.workshopContentDir` walk on a temp Steam library and **wins over `resources/` + the exe sibling**; `AC27_WORKSHOP_DIR` override wins over the game-root path and accepts an exe file (uses its dirname); fallbacks to `resources/`, the `PORTABLE_EXECUTABLE_FILE` sibling, and `app.getPath('exe')`; null when no candidate exists. |
...
| `electron/ptt-shortcut.test.js` | 26 | **Global PTT hotkey manager** (`electron/pttShortcut.js`, node env, injected `globalShortcut`/config/window fakes) — config defaults (absent/trimmed/disabled/non-string/throwing loader); set registers + persists (`pttShortcut` in `ac27-config.json`), re-register swaps the old accelerator, empty string disables without registering; register-failure/throw never persisted, save failure surfaced; hotkey fires the toggle on the focused strips window (first-window fallback, destroyed windows skipped, silent with none open); **hold-to-talk with an injected `keyWatch`: press fires `onPress` + arms the watcher (no toggle), key-up fires `onRelease`, auto-repeat re-arms without a second press, unavailable/unmappable falls back to toggle, dispose/remap while held releases the mic**; dispose unregisters (tolerates throws); throwing window-list/windows/trigger tolerated. |
| `electron/ptt-key-watch.test.js` | 9 | **Windows PTT key-UP watcher** (`electron/pttKeyWatch.js`, node env, injected `isDown`/fake timers) — `acceleratorToVk` (Space/P/1/Tab/F1–F24/OEM punctuation, 0 for empty/unmappable); `createPttKeyWatch` unavailable without a probe (`null` disables, `undefined` uses the platform default), `onUp` fires exactly once on the down→up transition, re-arming replaces the pending hold without firing the old callback, `cancel` stops silently, a throwing probe fails safe to released, an `onUp` throw is tolerated. |
| `electron/voice-stt-worker.test.js` | 5 | **Voice STT worker start/stop race** (`electron/voiceSttWorker.js`, node env, stubbed `_send`) — start sends `start` + clears `startPending` on `started`; a stop that races the start is remembered and applied once `started` arrives (drain then `stop`); normal stop drains then sends `stop`; idle stop no-op; re-press inside the drain cancels the pending stop. |
...
| **MapWindows (23 files):** | **760** | |
| `components/MapWindows/VoicePTTButton.test.jsx` | 7 | **PTT hold-to-talk button** — press/pointerup start/stop, **does not release on mouseLeave/mouseOut mid-hold**, duplicate presses/releases ignored (auto-repeat), pointercancel releases, keyboard Space hold, window-blur release, hidden when unsupported. |
...
| `components/MapWindows/FlightPatchCommandBar.test.jsx` | 15 | **"Fly Waypoint" composer** — option placement left of Clear for Approach, STAR waypoint picker left-to-right in route order, `update_heading` payload at the bearing to the picked fix, Fly Waypoint → Fly Heading override, Clear for Approach supersede, Send/Cancel colors |
...
| `components/MapWindows/PttShortcutModal.test.jsx` | 15 | **Global PTT hotkey remap modal** — loads/shows the saved shortcut; live key echo (held modifiers show `Shift+…`, settles on release); capture + persist a combo, Backspace disables (`Off`); Escape/X/backdrop close (inner clicks don't), missing-`onClose` tolerance; register-failure red flash + `[PTT-SHORTCUT]` main-log line; in-flight-save guard (second key ignored); missing-bridge red flag; modifier-only with no mods falls back to the saved value. |
...
| SetupScreen / LanguagePicker | Workshop build auto-detects the game root and offers it; the normal (non-Workshop) build never searches and requires manual folder selection; first launch with no stored language shows the picker before any other UI, and a chosen language persists to localStorage + cache.json. |
...
|------|-------|-------------------|----------|
| `test_api_server.js` | 145 | All 7 HTTP endpoints (status, airport/values, flights, create-batch, modify-batch, delete-batch, validation) + MCP protocol (initialize, tools/list, 7 tools/call) + validation suite (airline, flight number, stand, runway, STAR compat, registration pair, time bounds, time order, duplicate callsigns, stand conflicts, duplicate registrations, **runway inactive at landing** via chronological `initialRunways` + `timeline` active-set sweep; aircraft type is airline-independent; **airline/language** — `buildConstraints` carries `airlineCountries`/`voiceLanguages`/`languages` from the renderer's `airportValues`, `airlineLanguageMap` resolves CCA→zh / AAL·DAL→en (CCA→en at an en-only airport), `validateFlightObjects` rejects `airline_language_mismatch` and accepts the correct combo, and `applyCascades` mirrors Cascade 2b) + cascade logic + AND-match regression tests. Mock Electron window — no real app needed. | 145/145 pass |
...
| `e2e/fuzz-save.spec.mjs` | Fuzz save test — `FuzzTest(aclFilePath, opts)` exported; MCP randomized ops + UI save-with-backup per level (gated on `FUZZ_RUN=1`). Gate includes `voice-language-mismatch` + `airline-language-mismatch`; `FUZZ_REPLACE=1`/`--replace` writes passed levels back to `E2E_GAME_ROOT`; `FUZZ_ACL_FILES` targets a subset (browser-visible levels only) |
