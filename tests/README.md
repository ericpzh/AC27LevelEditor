...
```bash
npm run test:all      # Full suite: Vitest (1497) + save integrity (27) + jetway rebuild (27) + runway pairs (5) + E2E (18, ~8 min)
npm test              # Vitest component + store + utility + electron + integration + MapWindow + updater tests (1497 tests, 87 files, ~10s)
...
## Layer 1 — Vitest Component Tests (1497 tests, 87 files)
...
### `npm test` — 1497 pass (87 test files; includes the Ground Painter scenery suite + airway roundtrip)
...
Drives the Ground Painter the same way `F1` drives flights: opens each level, opens the Ground Painter, applies **50–200 randomized ops per level** through the MCP Ground/Air Painter API split across **two mode-partitioned phases**, then hits **SAVE through the real Ground Painter UI** (Save → backup confirm → success/warnings) — still in air mode, exercising the mode-independent save path. **Phase 1 (ground mode):** 7% runway (new/move/rename), 7% taxiway new, 25% taxiway mod (move whole/move endpoint/rename), 25% fillet (half connect a new taxiway onto a runway `Flags=4` strip then fillet that junction), 12% area (new/move/move-vertex), 6% stand (new/move/rename), 18% select+delete (single/multi/selectAll move + delete). **Mode swap:** the real toolbar air/ground toggle (`[data-testid="air-ground-toggle"]`, `set_ground_painter_mode` MCP fallback) is clicked and the run asserts `get_ground_painter_state().mode === 'air'`. **Phase 2 (air mode, air bounds):** 40% `create_airway_nodes`, 25% `create_airway_procedures`, 13% `create_airway_fillet` (intra-procedure AAA, radius 50..500 GU, MCP auto-picks the sharpest corner), 10% `move_airway_objects`, 7% `rename_airway_object`, 5% `delete_airway_objects`. Ground coordinates come from the live graph extents (5% padding); air coordinates from the authored airway-node extents (or a wide box around the ground center when the level has no fixes). Runway names are suffix-deduped and always satisfy the save-time validation. Post-save, the airside is gated: no non-finite airway node, every procedure has ≥2 distinct in-range fixes, and the save never adds air objects.
