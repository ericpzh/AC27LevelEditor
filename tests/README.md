...
| `utils/livery.test.js` | 9 | **Livery constants** — `OWN_PACK_NAME`, the 20-row `SHORT_CODE_TO_PLANE_ID` table and its reverse map, `LIVERY_FOLDER_SAFE_RE` (free-form names accepted incl. CJK/spaces; rejects traversal `../`, separators, Windows-reserved chars, leading/trailing dot/space, >64 chars), `folderFor` conventional default (unknown plane id falls back to the id itself), `buildManifest` template + free-form folder id sanitization (`My First Livery 01` → `my_first_livery_01_default` while keeping the structured display name/airline), `TEXTURE_SIZE` = 2048. |
