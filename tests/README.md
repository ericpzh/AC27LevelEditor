...
```bash
npm run test:all      # Full suite: Vitest (1655) + save integrity (27) + jetway rebuild (27) + runway pairs (5) + E2E (18, ~8 min)
npm test              # Vitest component + store + utility + electron + integration + MapWindow + updater tests (1655 tests, 98 files, ~24s)
...
## Layer 1 — Vitest Component Tests (1655 tests, 98 files)
...
### `npm test` — 1655 pass (98 test files; includes the Ground Painter scenery suite + airway roundtrip + the full Livery page suite)
...
| `components/LiveryScreen/LiveryScreen.test.jsx` | 17 | **Livery shell** — header hosts Back/Pack (LHS) + New/Select All/Export/Delete/search/help (RHS, Export/Delete icon + label), old tabbar/bottombar gone; search input renders (no redundant tooltip); Back→browser; New opens the painter (`Save As`, `.livery-canvas-wrap`) and its Back returns to the list; header select-all + batch delete end-to-end; Export/Delete stay disabled until a selection (Export needs exactly one); search filters; Pack opens the install modal + download overlay and shows the Mods target; help overlay opens with Views/Header bar/Paint tools (Views lists only the documented chips) and closes via Escape/backdrop; header tooltips (New exempt); **unsaved guard:** a dirty `window.__liveryPaintGuard` blocks New + Back with the Unsaved Changes modal until Discard; clicking a card opens the painter prefilled; install modal Close dismisses. |
