...
```bash
npm run test:all      # Full suite: Vitest (recount via npm test; last verified 1203) + save integrity (21) + jetway rebuild (16) + runway pairs (5) + E2E (17, ~5 min)
npm test              # Vitest component + store + utility + electron + MapWindow + updater tests (~7s; count stale — recount)
...
### `npm test` — run to get the current pass count (`58` test files; last verified `1203`, now stale — the in-flight Ground-Painter WIP plus the two new save regressions `animator_lean_scope`/`zeroflight_save` were added after that count)
...
All Vitest tests pass (58 test files; count stale after the Ground-Painter WIP + new save regressions — recount via `npm test`). The previously failing/todo items have been fixed:
