...
```bash
npm run test:all      # Full suite: Vitest (1687) + save integrity (27) + jetway rebuild (27) + runway pairs (5) + E2E (18, ~8 min)
npm test              # Vitest component + store + utility + electron + integration + MapWindow + updater tests (1687 tests, 99 files, ~24s)
...
## Layer 1 — Vitest Component Tests (1687 tests, 99 files)
...
### `npm test` — 1687 pass (99 test files; includes the Ground Painter scenery suite + airway roundtrip + the full Livery page suite)
...
| **Livery page (custom liveries):** | **171** | |
...
| `components/LiveryScreen/LiveryCanvas.test.jsx` | 25 | **Paint canvas** — tool rail single-active switching; brush stroke (undo snapshot + stroke/lineTo); eraser; eyedropper picks the pixel colour and returns to brush; flood Fill commits via `putImageData` + tolerance slider; line/rect/ellipse draw on pointer-up + width/fill controls; text commit flattens to `fillText`; text font/size/bold/italic options; brush opacity slider + hard/soft (soft sets a shadow blur); options bar stays mounted for option-less tools; true-size cursor ring (show/hide, size × zoom); undo/redo replay snapshots; Clear confirm/cancel; keyboard tool shortcuts + Ctrl+Z; zoom in/out/fit readout; sticker import/duplicate/remove, **Flip Horizontal/Vertical** (buttons disabled without a sticker; flipping drives `ctx.scale(-1, 1)` on the next draw), export flattening it over a transparent background, Escape deselect (stays removable), failed sticker read toast (BAD_IMAGE); save payload is a 2048 PNG data-URL. |
