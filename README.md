...
**3D livery preview:** a `LuRotate3D` button right of the aircraft selector (hidden for types with no HD model — the Cessna Citation X) renders the current livery on the game's **real aircraft geometry** in a floating window (drag to orbit, wheel to zoom, header drag to move, corner grip to resize, bottom-right button to reset the camera, top-right to hide). The mesh is the game's copyrighted content, so it is never shipped: when the livery page opens, the editor extracts the geometry **in-process** from the user's own `<dataRoot>/resources.assets` (a pure-JS Unity serialized-file reader, `electron/unity/` — no Python, no spawned binary, cross-platform by construction; the AC27 build strips type trees, so field layouts come from `electron/unity/typetrees.json`), caches a compact binary pack (manifest + per-plane positions/UVs/indices, ~16 MB for all types) under `<userData>/livery-3d-models/`, and deletes it when the livery page is left. `scripts/extract-aircraft-models.py` (UnityPy) is kept as a last-resort fallback if a game update changes Unity's serialization and a Python with `UnityPy`/`numpy` is installed; otherwise a failed extraction toasts and the list/preview simply falls back to no preview. While the pack is available, each livery card's preview is a **3D snapshot** of that livery on its aircraft (rendered offscreen; the resized PNG thumbnail channel is no longer used — the full livery texture is the model's map). The pack is keyed by plane id and covers all 19 shipped types, with per-type livery-part mapping (`Fuselage`/`Wing` for the A388, `Body` elsewhere); mesh world transforms are baked at extraction so every aircraft comes out Y-up, and non-livery parts (engines/fans) render in flat grey.
