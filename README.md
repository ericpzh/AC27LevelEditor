...
**第一步：选择关卡** — 浏览器页面按机场分组显示生产关卡和 30 分钟 Demo 关卡。教程/测试/无尽/开发等变体默认隐藏。每个卡片显示关卡时间范围、航班数量等信息。Demo 模式仅显示 `.demo.acl` 切片关卡。
...
**Step 1 — Browse:** Production levels and 30-minute `.demo.acl` slices are listed, grouped by airport. Tutorial/test/endless/dev variants are hidden. Each card shows time range, flight count, and metadata. Demo mode shows only `.demo.acl` levels.
